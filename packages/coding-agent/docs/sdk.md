> daedalus can help you use the SDK. Ask it to build an integration for your use case.
...
```typescript
import { createAgentSession } from "@daedalus-pi/coding-agent";
...
The `Agent` class (from `@daedalus-pi/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.daedalus/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.daedalus/extensions/`)
- Project skills:
  - `.daedalus/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.daedalus/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.daedalus/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.daedalus/agent/auth.json and ~/.daedalus/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
- If you omit `tools`, daedalus automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "@daedalus-pi/coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.daedalus/agent/extensions/`, `.daedalus/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@daedalus-pi/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@daedalus-pi/coding-agent";
...
1. Global: `~/.daedalus/agent/settings.json`
2. Project: `<cwd>/.daedalus/settings.json`
