> daedalus can create extensions. Ask it to build one for your use case.
...
Create `~/.daedalus/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.daedalus/agent/extensions/*.ts` | Global (all projects) |
| `~/.daedalus/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.daedalus/extensions/*.ts` | Project-local |
| `.daedalus/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as daedalus packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@daedalus-pi/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `@sinclair/typebox` | Schema definitions for tool parameters |
| `@daedalus-pi/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@daedalus-pi/tui` | TUI components for custom rendering |
...
After a successful switch or new-session action, daedalus emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
After a successful fork, daedalus emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
Before `tool_call` runs, daedalus waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@daedalus-pi/coding-agent";
...
```typescript
import { isBashToolResult } from "@daedalus-pi/coding-agent";
...
```typescript
import { createLocalBashOperations } from "@daedalus-pi/coding-agent";
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while daedalus is idle.
...
```typescript
import { SessionManager } from "@daedalus-pi/coding-agent";
...
If multiple extensions register the same command name, daedalus keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "@daedalus-pi/tui";
...
If your custom tool mutates files, use `withFileMutationQueue()` so it participates in the same per-file queue as built-in `hashline_edit`, `edit`, and `write`. This matters because tool calls run in parallel by default. Without the queue, two tools can read the same old file contents, compute different updates, and then whichever write lands last overwrites the other.

Example failure case: your custom tool edits `foo.ts` while built-in `hashline_edit` or `edit` also changes `foo.ts` in the same assistant turn. If your tool does not participate in the queue, both can read the original `foo.ts`, apply separate changes, and one of those changes is lost.
...
```typescript
import { withFileMutationQueue } from "@daedalus-pi/coding-agent";
...
**Important:** Use `StringEnum` from `@daedalus-pi/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when daedalus resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
- [ls.ts](https://github.com/Likas07/Daedalus/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createBashTool } from "@daedalus-pi/coding-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/Likas07/Daedalus/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@daedalus-pi/coding-agent";
...
```typescript
import { Text, Component } from "@daedalus-pi/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@daedalus-pi/coding-agent";
import { matchesKey } from "@daedalus-pi/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@daedalus-pi/coding-agent";
