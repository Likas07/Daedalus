> daedalus can create TUI components. Ask it to build one for your use case.
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@daedalus-pi/tui";
...
```typescript
import { Container, type Focusable, Input } from "@daedalus-pi/tui";
...
Import from `@daedalus-pi/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@daedalus-pi/tui";
```
...
```typescript
import { matchesKey, Key } from "@daedalus-pi/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@daedalus-pi/tui";
...
```typescript
import { getMarkdownTheme } from "@daedalus-pi/coding-agent";
import { Markdown } from "@daedalus-pi/tui";
...
For letting users pick from a list of options. Use `SelectList` from `@daedalus-pi/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@daedalus-pi/coding-agent";
import { DynamicBorder } from "@daedalus-pi/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@daedalus-pi/tui";
...
For toggling multiple settings. Use `SettingsList` from `@daedalus-pi/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@daedalus-pi/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@daedalus-pi/tui";
...
import { matchesKey, truncateToWidth } from "@daedalus-pi/tui";
