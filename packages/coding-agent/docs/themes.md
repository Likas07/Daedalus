> daedalus can create themes. Ask it to build one for your setup.
...
On first run, daedalus detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.daedalus/agent/themes
vim ~/.daedalus/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Likas07/Daedalus/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, daedalus reloads it automatically for immediate visual feedback.
...
Daedalus uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, daedalus falls back to the nearest approximation.
