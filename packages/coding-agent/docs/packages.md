...
If no `daedalus` manifest is present, daedalus auto-discovers resources from these directories:
...
Use `daedalus config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.daedalus/agent`) and project (`.daedalus/`) scopes.
