...
function isOfflineModeEnabled(): boolean {
	const value = process.env.DAEDALUS_OFFLINE;
...
function readDaedalusManifestFile(packageJsonPath: string): DaedalusManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { daedalus?: DaedalusManifest; pi?: DaedalusManifest };
		return pkg.daedalus ?? pkg.pi ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readDaedalusManifestFile(packageJsonPath);
...
export class DefaultPackageManager implements PackageManager {
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "daedalus-extensions", private: true };
	...
	private getTemporaryDir(prefix: string, suffix?: string): string {
		const hash = createHash("sha256")
			...
			.digest("hex")
			.slice(0, 8);
		return join(tmpdir(), "daedalus-extensions", prefix, hash, suffix ?? "");
	}
	...
	private collectPackageResources(
		...
		const manifest = this.readDaedalusManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof DaedalusManifest];
	...
	private readDaedalusManifest(packageRoot: string): DaedalusManifest | null {
