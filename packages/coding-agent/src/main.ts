...
import { type ImageContent, modelsAreEqual, supportsXhigh } from "@daedalus-pi/ai";
import { ProcessTerminal, setKeybindings, TUI } from "@daedalus-pi/tui";
...
export async function main(args: string[]) {
	...
	if (offlineMode) {
		...
		process.env.DAEDALUS_SKIP_VERSION_CHECK = "1";
	}
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.DAEDALUS_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: DAEDALUS_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
