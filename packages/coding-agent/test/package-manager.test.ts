...
describe("DefaultPackageManager", () => {
	...
	let globalStubs = createGlobalStubManager();
	beforeEach(() => {
		previousOfflineEnv = process.env.DAEDALUS_OFFLINE;
		delete process.env.DAEDALUS_OFFLINE;
		...
		globalStubs = createGlobalStubManager();
	});
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.DAEDALUS_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
...
Content`,
	...
	describe("source parsing", () => {
		...
		it("should recognize github URLs without git: prefix", async () => {
			...
			const runCommandSpy = vi
				...
				.mockRejectedValue(new Error("clone blocked by test"));

			// This should be parsed as a git source, not throw "unsupported" before clone.
			await expect(packageManager.install("https://github.com/nonexistent/repo")).rejects.toThrow(
				"clone blocked by test",
			);
			...
			expect(events.some((e) => e.type === "start" && e.action === "install")).toBe(true);
			expect(events.some((e) => e.type === "error" && e.message === "clone blocked by test")).toBe(true);
		});
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to the project config dir", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		...
		it("should report updates for installed unpinned npm packages", async () => {
			...
			globalStubs.stub("fetch", fetchMock);
