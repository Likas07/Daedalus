...
describe("status dashboard", () => {
	...
	it("status_overview reports workspace readiness and todo summary", async () => {
		...
		const result = await tool!.execute("status-overview-1", {}, undefined, undefined, { cwd: tempDir, model: getModel("anthropic", "claude-sonnet-4-5")!, hasPendingMessages: () => false, sessionManager } as any);
	...
	it("/status command emits a status dashboard custom message", async () => {
		...
		const statusMessages = session.messages.filter((message) => message.role === "custom" && message.customType === "status-dashboard");
