...
describe("status dashboard", () => {
	...
	it("status_overview does not mention semantic workspace when semantic search is disabled", async () => {
		...
		expect(text).not.toContain("semantic_workspace:");
	...
	it("/status command emits a status dashboard custom message without semantic state", async () => {
		const settingsManager = SettingsManager.create(tempDir, agentDir);
		const sessionManager = SessionManager.inMemory();
