...
function db(): AppServerDatabase { database = openAppServerDatabase(":memory:"); runMigrations(database); return database; }
function entry(text: string): SessionEntry { return { type: "message", id: crypto.randomUUID(), parentId: null, timestamp: "2026-04-26T00:00:01.000Z", message: { role: "user", content: [{ type: "text", text }], timestamp: 0 } }; }

describe("ExportService", () => {
	test("redacts API keys, bearer tokens, OAuth tokens, and auth headers", () => {
		expect(redactSecrets({ Authorization: "Bearer abc123", text: "api_key=sk-secret oauth_token=tok Bearer live" })).toEqual({ Authorization: "[REDACTED]", text: "api_key=[REDACTED] oauth_token=[REDACTED] Bearer [REDACTED]" });
	});

	test("exports JSONL from SQLite and redacts session content", async () => {
		const database = db();
		const store = new SqliteSessionStore({ database });
		const content = serializeSessionJsonl({ header: { type: "session", version: 3, id: "s1", timestamp: "2026-04-26T00:00:00.000Z", cwd: "/repo" }, entries: [entry("Bearer secret-token")] });
	...
	test("exports HTML session", async () => {
		const database = db();
		await new SqliteSessionStore({ database }).import({ session: { header: { type: "session", version: 3, id: "s-html", timestamp: "2026-04-26T00:00:00.000Z", cwd: "/repo" }, entries: [entry("<hello>")] } });
	...
	test("limits recent events and builds support bundle", async () => {
		...
		const result = await new ExportService({ database, runtimeDiagnostics: () => ({ active: true }) }).export({ kind: "support-bundle", recentEventLimit: 1, includeToolLogs: true });
