...
function runnerFor(responses: Record<string, { stdout?: string; stderr?: string; exitCode: number }>): CommandRunner {
	return async (args) => {
		...
		return response
			? { stdout: response.stdout ?? "", stderr: response.stderr, exitCode: response.exitCode }
			: { stdout: "", stderr: `unexpected ${args.join(" ")}`, exitCode: 1 };
	};
}
...
describe("GitHubCliService", () => {
	test("returns actionable missing gh status", async () => {
		const service = new GitHubCliService({
			runner: runnerFor({ "gh --version": { stderr: "not found", exitCode: 127 } }),
		});
		expect(await service.status()).toEqual({
			status: "not-configured",
			code: "gh-missing",
			message: expect.stringContaining("Install gh"),
		});
	});
	...
	test("returns actionable unauthenticated status", async () => {
		const service = new GitHubCliService({
			runner: runnerFor({
				"gh --version": { stdout: "gh version", exitCode: 0 },
				"gh auth status": { stderr: "not logged in", exitCode: 1 },
			}),
		});
		expect(await service.status()).toEqual({
			status: "unauthenticated",
			code: "gh-auth",
			message: "Run `gh auth login` to connect GitHub.",
		});
	});
		...
		const service = new GitHubCliService({
			runner: runnerFor({
				"gh repo view --json owner,name,url": {
					stdout: JSON.stringify({ owner: { login: "acme" }, name: "app", url: "https://github.com/acme/app" }),
					exitCode: 0,
				},
				"gh issue list --limit 25 --json number,title,url,state,labels": {
					stdout: JSON.stringify([
						{ number: 3, title: "Bug", url: "https://issue", state: "OPEN", labels: [{ name: "bug" }] },
					]),
					exitCode: 0,
				},
				...
				"gh pr checks HEAD --json name,state,link,bucket,description": {
					stdout: JSON.stringify([{ name: "test", state: "SUCCESS", link: "https://ci", description: "ok" }]),
					exitCode: 0,
				},
			}),
		});
		expect(await service.repository()).toEqual({
			owner: "acme",
			name: "app",
			remoteUrl: "https://github.com/acme/app",
		});
		expect(await service.issues()).toEqual([
			{ id: "3", number: 3, title: "Bug", url: "https://issue", state: "open", labels: ["bug"] },
		]);
		expect(await service.pullRequests()).toEqual([
			{
				number: 7,
				title: "Fix",
				url: "https://pr",
	...
	test("creates and opens pull requests with gh", async () => {
		...
		expect(await service.createPullRequest({ title: "Ship", head: "feature", base: "main", body: "Body" })).toEqual({
			status: "created",
			number: 9,
			url: "https://pr/9",
		});
