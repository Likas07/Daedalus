...
test("shell snapshot preserves runsIn target and pending approval counts", () => {
	...
	database
		.query(
			"INSERT INTO sessions (id,project_id,worktree_id,status,title,runs_in_json,validation_status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)",
		)
		.run(
			"s1",
			"p1",
	...
	database
		...
		.run(JSON.stringify({ summary: "Approve tool" }));
...
test("shell snapshot labels base checkout and needs-attention reasons", () => {
	...
	database
		.query(
			"INSERT INTO sessions (id,project_id,status,title,runs_in_json,validation_status,needs_attention_reason,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)",
		)
		.run(
			"s2",
			"p1",
