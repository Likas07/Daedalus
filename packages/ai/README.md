...
```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@daedalus-pi/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@daedalus-pi/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@daedalus-pi/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@daedalus-pi/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@daedalus-pi/ai';
...
```typescript
import { getModel, stream } from '@daedalus-pi/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@daedalus-pi/ai';
...
```typescript
import { Model, stream } from '@daedalus-pi/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@daedalus-pi/ai';
...
```typescript
import { getModel, complete, Context } from '@daedalus-pi/ai';
...
```typescript
import { Context, getModel, complete } from '@daedalus-pi/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@daedalus-pi/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@daedalus-pi/ai';
...
```bash
npx @daedalus-pi/ai login              # interactive provider selection
npx @daedalus-pi/ai login anthropic    # login to specific provider
npx @daedalus-pi/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@daedalus-pi/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@daedalus-pi/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@daedalus-pi/ai/oauth';
...
```typescript
import { getModel, complete } from '@daedalus-pi/ai';
import { getOAuthApiKey } from '@daedalus-pi/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@daedalus-pi/ai`
