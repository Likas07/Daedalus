...
class FixedFrameComponent implements Component {
	historyStart = 0;
	spinner = "|";
	showStatus = true;
	...
	render(width: number): string[] {
		const lines: string[] = [];
		const historyRows = Math.max(0, this.frameHeight - 3);
		for (let i = 0; i < historyRows; i++) {
			lines.push(`History ${this.historyStart + i}`);
		}
		lines.push(this.showStatus ? `Working ${this.spinner}` : "");
		lines.push("Composer");
		lines.push("Footer");
...
function getBufferState(terminal: VirtualTerminal): { length: number; viewportY: number; activeType?: string } {
	const xterm = (
		terminal as unknown as {
			xterm: XtermTerminalType & { buffer: { active: { length: number; viewportY: number; type?: string } } };
		}
	).xterm;
	return {
		length: xterm.buffer.active.length,
		viewportY: xterm.buffer.active.viewportY,
		activeType: xterm.buffer.active.type,
	};
}
...
describe("TUI alternate screen", () => {
	it("restores primary screen content after interactive full-screen session exits", async () => {
		const terminal = new VirtualTerminal(30, 8, { alternateScreen: true });
		terminal.write("shell line 1\r\nshell line 2");
		await flushRender(terminal);
		const shellViewport = terminal.getViewport();

		const tui = new TUI(terminal);
		tui.setFixedFrameMode(true);
		const component = new FixedFrameComponent(terminal.rows);
		tui.addChild(component);
		tui.start();
		await flushRender(terminal);

		const duringViewport = terminal.getViewport();
		assert.ok(
			duringViewport.some((line) => line.includes("History 0")),
			"TUI content should render in alternate screen",
		);
		assert.ok(
			duringViewport.every((line) => !line.includes("shell line 1")),
			"Primary screen content should be hidden while TUI is active",
		);

		const spinnerFrames = ["|", "/", "-", "\\"];
		for (let i = 0; i < 16; i++) {
			component.historyStart = i;
			component.spinner = spinnerFrames[i % spinnerFrames.length] ?? "|";
			component.showStatus = i % 2 === 0;
			tui.requestRender();
			await flushRender(terminal);
		}
		...
		assert.deepStrictEqual(terminal.getViewport(), shellViewport);
	});
});
...
describe("TUI differential rendering", () => {
	it("keeps fixed-frame updates inside the visible viewport during render churn", async () => {
		const terminal = new VirtualTerminal(30, 8, { alternateScreen: true });
		const tui = new TUI(terminal);
		tui.setFixedFrameMode(true);
		const component = new FixedFrameComponent(terminal.rows);
		...
		const initialBuffer = getBufferState(terminal);
		assert.strictEqual(initialBuffer.length, terminal.rows);
		assert.strictEqual(initialBuffer.viewportY, 0);
		assert.strictEqual(initialBuffer.activeType, "alternate");

		const spinnerFrames = ["|", "/", "-", "\\"];
		for (let i = 0; i < 24; i++) {
			component.historyStart = i;
			component.spinner = spinnerFrames[i % spinnerFrames.length] ?? "|";
			component.showStatus = i % 3 !== 0;
			tui.requestRender();
			await flushRender(terminal);

			const viewport = terminal.getViewport();
			const buffer = getBufferState(terminal);
			assert.strictEqual(buffer.length, terminal.rows, `Scrollback grew on iteration ${i}`);
			assert.strictEqual(buffer.viewportY, 0, `Viewport scrolled on iteration ${i}`);
			for (let row = 0; row < terminal.rows - 3; row++) {
				assert.ok(!viewport[row]?.includes("Working"), `Stale status line at row ${row}: ${viewport[row]}`);
			}
			assert.ok(viewport[terminal.rows - 2]?.includes("Composer"), `Composer moved: ${viewport[terminal.rows - 2]}`);
			assert.ok(viewport[terminal.rows - 1]?.includes("Footer"), `Footer moved: ${viewport[terminal.rows - 1]}`);
		}
