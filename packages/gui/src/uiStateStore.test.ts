...
describe("uiStateStore pure functions", () => {
	...
	it("syncProjects returns a new state when only project cwd changes", () => {
		...
		const next = syncProjects(initialState, [{ key: project1, logicalKey: project1, cwd: "/tmp/project-1-renamed" }]);
...
describe("uiStateStore persistence round-trip", () => {
	...
	it("preserves all-collapsed project state across restart", () => {
		...
		const persisted = JSON.parse(localStorageStub.getItem(PERSISTED_STATE_KEY) ?? "{}") as PersistedUiState;
	...
	it("preserves expand state across restart when project's logical key changes", () => {
		...
		let state = syncProjects(makeUiState(), [{ key: physicalKey, logicalKey: previousLogicalKey, cwd }]);
		...
		const rehydrated = syncProjects(makeUiState(), [{ key: physicalKey, logicalKey: nextLogicalKey, cwd }]);
