...
describe("splitPromptIntoComposerSegments", () => {
	...
	it("keeps inline terminal context placeholders at their prompt positions", () => {
		expect(
			splitPromptIntoComposerSegments(`Inspect ${INLINE_TERMINAL_CONTEXT_PLACEHOLDER}@AGENTS.md please`),
		).toEqual([
			{ type: "text", text: "Inspect " },
...
describe("selectionTouchesMentionBoundary", () => {
	...
	it("returns true when selection includes the whitespace before a mention", () => {
		expect(
			selectionTouchesMentionBoundary("hi there @package.json later", "hi there".length, "hi there ".length),
		).toBe(true);
	});
