...
const PersistedComposerThreadDraftState = Schema.Struct({
	...
	modelSelectionByProvider: Schema.optionalKey(Schema.Record(ProviderKind, Schema.optionalKey(ModelSelection))),
 ...
 */
type ProviderOptionSelectionsByProvider = Partial<Record<ProviderKind, ReadonlyArray<ProviderOptionSelection>>>;
...
const PersistedComposerDraftStoreState = Schema.Struct({
	...
	stickyModelSelectionByProvider: Schema.optionalKey(Schema.Record(ProviderKind, Schema.optionalKey(ModelSelection))),
...
interface ComposerDraftStoreState {
	...
	clearProjectDraftThreadById: (projectRef: ScopedProjectRef, threadRef: ComposerThreadTarget) => void;
	...
	setModelSelection: (threadRef: ComposerThreadTarget, modelSelection: ModelSelection | null | undefined) => void;
	/** Replace the model options for one or more providers in the draft. */
	setModelOptions: (
		threadRef: ComposerThreadTarget,
		modelOptions: Partial<Record<ProviderKind, ReadonlyArray<ProviderOptionSelection>>> | null | undefined,
	) => void;
	...
	setRuntimeMode: (threadRef: ComposerThreadTarget, runtimeMode: RuntimeMode | null | undefined) => void;
	setInteractionMode: (
		threadRef: ComposerThreadTarget,
	...
	syncPersistedAttachments: (threadRef: ComposerThreadTarget, attachments: PersistedComposerImageAttachment[]) => void;
...
const EMPTY_MODEL_SELECTION_BY_PROVIDER: Partial<Record<ProviderKind, ModelSelection>> = Object.freeze({});
...
function normalizeProviderKind(value: unknown): ProviderKind | null {
	return value === "codex" || value === "claudeAgent" || value === "cursor" || value === "opencode" ? value : null;
}
 ...
 * ignored downstream.
 */
function coerceProviderOptionSelections(value: unknown): ReadonlyArray<ProviderOptionSelection> | undefined {
...
function normalizeProviderModelOptions(
	...
	if (provider === "codex" && legacy) {
		...
		const fastMode =
			legacy.codexFastMode === true || (typeof legacy.serviceTier === "string" && legacy.serviceTier === "fast");
		if (fastMode) {
...
function legacyReplaceProviderModelOptions(
	...
	const { [provider]: _discardedProviderModelOptions, ...otherProviderModelOptions } = currentModelOptions ?? {};
		...
		for (const provider of ["codex", "claudeAgent", "cursor", "opencode"] as const) {
			...
			if (options && options.length > 0) {
				result[provider] = createModelSelection(
					provider,
					modelSelection?.provider === provider ? modelSelection.model : DEFAULT_MODEL_BY_PROVIDER[provider],
					options,
				);
			}
		}
...
export function deriveEffectiveComposerModelState(input: {
	draft: Pick<ComposerThreadDraftState, "modelSelectionByProvider" | "activeProvider"> | null | undefined;
	...
	const selectedModel = activeSelection?.model
		? resolveAppModelSelection(input.selectedProvider, input.settings, input.providers, activeSelection.model)
		: baseModel;
	const modelOptions =
...
function normalizePersistedTerminalContextDraft(value: unknown): PersistedTerminalContextDraft | null {
	if (!value || typeof value !== "object") {
		return null;
	}
	const candidate = value as Record<string, unknown>;
	...
	const terminalLabel = typeof candidate.terminalLabel === "string" ? candidate.terminalLabel.trim() : "";
	if (terminalId.length === 0 || terminalLabel.length === 0) {
		return null;
	}
...
function normalizeDraftThreadEnvMode(value: unknown, fallbackWorktreePath: string | null): DraftThreadEnvMode {
...
type ComposerThreadLookupState = Pick<ComposerDraftStoreState, "draftsByThreadKey" | "draftThreadsByThreadKey">;
...
function resolveComposerDraftKey(state: ComposerThreadLookupState, target: ComposerThreadTarget): string | null {
...
function resolveComposerThreadId(state: ComposerThreadLookupState, target: ComposerThreadTarget): ThreadId | null {
	const normalizedTarget = normalizeComposerTarget(state, target);
	if (!normalizedTarget) {
		return null;
	}
...
function toProjectDraftSession(draftId: DraftId, draftSession: DraftSessionState): ProjectDraftSession {
	return {
		draftId,
		...draftSession,
	};
...
function createDraftThreadState(
	...
	const projectChanged =
		...
		(existingThread.environmentId !== projectRef.environmentId || existingThread.projectId !== projectRef.projectId);
	...
	return {
		...
		envMode:
			options?.envMode ??
			(nextWorktreePath ? "worktree" : projectChanged ? "local" : (existingThread?.envMode ?? "local")),
		promotedTo: null,
	};
}
...
function removeDraftThreadReferences(
	state: Pick<
		...
		"draftThreadsByThreadKey" | "draftsByThreadKey" | "logicalProjectDraftThreadKeyByLogicalProjectKey"
	>,
	threadKey: string,
): Pick<
	ComposerDraftStoreState,
...
> {
	...
	const { [threadKey]: _removedDraftThread, ...restDraftThreadsByThreadKey } = state.draftThreadsByThreadKey;
...
> {
	...
	if (rawProjectDraftThreadIdByProjectKey && typeof rawProjectDraftThreadIdByProjectKey === "object") {
	...
	if (rawDraftThreadsByThreadId && typeof rawDraftThreadsByThreadId === "object") {
		for (const [threadKeyOrId, rawDraftThread] of Object.entries(
			...
			const threadId =
				...
				(typeof candidateDraftThread.threadId === "string" && candidateDraftThread.threadId.length > 0
					? (candidateDraftThread.threadId as ThreadId)
					: (threadKeyOrId as ThreadId));
			const environmentId =
				parsedThreadRef?.environmentId ??
				(typeof candidateDraftThread.environmentId === "string" && candidateDraftThread.environmentId.length > 0
			...
			const promotedTo =
				...
				promotedToRecord.threadId.length > 0
					? scopeThreadRef(promotedToRecord.environmentId as EnvironmentId, promotedToRecord.threadId as ThreadId)
					: null;
			...
			draftThreadsByThreadKey[threadKey] = {
				...
				interactionMode:
					candidateDraftThread.interactionMode === "plan" || candidateDraftThread.interactionMode === "default"
...
function normalizePersistedDraftsByThreadId(
	...
	for (const [threadKey, draftThread] of Object.entries(draftThreadsByThreadKey)) {
		...
		environmentIdByThreadId.set(parsedThreadRef.threadId, draftThread.environmentId as EnvironmentId);
	}

	const nextDraftsByThreadKey: DeepMutable<PersistedComposerDraftStoreState["draftsByThreadKey"]> = {};
	for (const [threadKeyOrId, draftValue] of Object.entries(rawDraftMap as Record<string, unknown>)) {
		if (typeof threadKeyOrId !== "string" || threadKeyOrId.length === 0) {
			continue;
		}
		...
		const terminalContexts = Array.isArray(draftCandidate.terminalContexts)
			...
			: [];
		const runtimeMode = isRuntimeMode(draftCandidate.runtimeMode) ? draftCandidate.runtimeMode : null;
		...
		if (draftCandidate.modelSelectionByProvider && typeof draftCandidate.modelSelectionByProvider === "object") {
			// v3 format
			...
			const normalizedModelOptions =
				normalizeProviderModelOptions(legacyDraftCandidate.modelOptions, undefined, legacyDraftCandidate) ?? null;
			...
			const modelSelection = legacySyncModelSelectionOptions(normalizedModelSelection, mergedModelOptions);
...
function migratePersistedComposerDraftStoreState(persistedState: unknown): PersistedComposerDraftStoreState {
	...
	const rawDraftThreadsByThreadId = candidate.draftThreadsByThreadKey ?? candidate.draftThreadsByThreadId;
	...
	const stickyModelSelection = legacySyncModelSelectionOptions(normalizedStickyModelSelection, nextStickyModelOptions);
	...
	const { draftThreadsByThreadKey, logicalProjectDraftThreadKeyByLogicalProjectKey } = normalizePersistedDraftThreads(
...
function partializeComposerDraftStoreState(state: ComposerDraftStoreState): PersistedComposerDraftStoreState {
	const persistedDraftsByThreadKey: DeepMutable<PersistedComposerDraftStoreState["draftsByThreadKey"]> = {};
	for (const [threadKey, draft] of Object.entries(state.draftsByThreadKey)) {
		if (typeof threadKey !== "string" || threadKey.length === 0) {
			continue;
		}
		const hasModelData = Object.keys(draft.modelSelectionByProvider).length > 0 || draft.activeProvider !== null;
		if (
			draft.prompt.length === 0 &&
			draft.persistedAttachments.length === 0 &&
			draft.terminalContexts.length === 0 &&
	...
	return {
		...
		logicalProjectDraftThreadKeyByLogicalProjectKey: state.logicalProjectDraftThreadKeyByLogicalProjectKey,
		stickyModelSelectionByProvider: state.stickyModelSelectionByProvider,
		stickyActiveProvider: state.stickyActiveProvider,
	};
}

function normalizeCurrentPersistedComposerDraftStoreState(persistedState: unknown): PersistedComposerDraftStoreState {
	if (!persistedState || typeof persistedState !== "object") {
		return EMPTY_PERSISTED_DRAFT_STORE_STATE;
	}
	const normalizedPersistedState = persistedState as LegacyPersistedComposerDraftStoreState;
	const { draftThreadsByThreadKey, logicalProjectDraftThreadKeyByLogicalProjectKey } = normalizePersistedDraftThreads(
		normalizedPersistedState.draftThreadsByThreadKey ?? normalizedPersistedState.draftThreadsByThreadId,
	...
	if (
		...
		stickyModelSelectionByProvider = normalizedPersistedState.stickyModelSelectionByProvider as Partial<
			Record<ProviderKind, ModelSelection>
		>;
		stickyActiveProvider = normalizeProviderKind(normalizedPersistedState.stickyActiveProvider);
	} else {
		// Legacy migration path
		const stickyModelOptions = normalizeProviderModelOptions(normalizedPersistedState.stickyModelOptions) ?? {};
		const normalizedStickyModelSelection = normalizeModelSelection(normalizedPersistedState.stickyModelSelection, {
		...
		stickyModelSelectionByProvider = legacyToModelSelectionByProvider(stickyModelSelection, nextStickyModelOptions);
...
function readPersistedAttachmentIdsFromStorage(threadKey: string): string[] {
	...
	try {
		const persisted = getLocalStorageItem(COMPOSER_DRAFT_STORAGE_KEY, PersistedComposerDraftStoreStorage);
		if (!persisted || persisted.version !== COMPOSER_DRAFT_STORAGE_VERSION) {
			return [];
		}
		return (persisted.state.draftsByThreadKey[threadKey]?.attachments ?? []).map((attachment) => attachment.id);
	} catch {
		return [];
	}
}
...
function verifyPersistedAttachments(
	...
	set: (
		partial:
			...
			| ((state: ComposerDraftStoreState) => ComposerDraftStoreState | Partial<ComposerDraftStoreState>),
		replace?: false,
	) => void,
): void {
...
function hydratePersistedComposerImageAttachment(attachment: PersistedComposerImageAttachment): File | null {
...
function toHydratedThreadDraft(persistedDraft: PersistedComposerThreadDraftState): ComposerThreadDraftState {
...
function toHydratedDraftThreadState(persistedDraftThread: PersistedDraftThreadState): DraftThreadState {
	return {
		...
		logicalProjectKey:
			...
			projectDraftKey(
				scopeProjectRef(persistedDraftThread.environmentId as EnvironmentId, persistedDraftThread.projectId),
			),
		...
		(setBase, get) => {
			...
			return {
				...
				getDraftSessionByLogicalProjectKey: (logicalProjectKey) => {
					...
					const draftId = get().logicalProjectDraftThreadKeyByLogicalProjectKey[normalizedLogicalProjectKey];
					if (!draftId) {
						return null;
					}
				...
				setProjectDraftThreadId: (projectRef, draftId, options) => {
					get().setLogicalProjectDraftThreadId(projectDraftKey(projectRef), projectRef, draftId, options);
				},
				setDraftThreadContext: (threadRef, options) => {
					...
					set((state) => {
						...
						const nextDraftThread: DraftThreadState = {
							...
							envMode:
								options.envMode ??
								(nextWorktreePath ? "worktree" : projectChanged ? "local" : (existing.envMode ?? "local")),
				...
				markDraftThreadPromoting: (threadRef, promotedTo) => {
					...
					set((state) => {
						...
						const nextPromotedTo = promotedTo ?? scopeThreadRef(existing.environmentId, existing.threadId);
				...
				addTerminalContext: (threadRef, context) => {
					...
					get().addTerminalContexts(typeof threadRef === "string" ? DraftId.make(threadKey) : threadRef, [
						context,
					]);
				},
		...
		},
		{
			...
			merge: (persistedState, currentState) => {
				...
				const draftThreadsByThreadKey = Object.fromEntries(
					Object.entries(normalizedPersisted.draftThreadsByThreadKey).map(([threadKey, draftThread]) => [
						threadKey,
...
export function useComposerDraftModelState(threadRef: ComposerThreadTarget): ComposerDraftModelState {
...
export function markPromotedDraftThreadByRef(threadRef: ScopedThreadRef): void {
	...
	for (const [draftId, draftThread] of Object.entries(draftStore.draftThreadsByThreadKey)) {
		if (draftThread.environmentId === threadRef.environmentId && draftThread.threadId === threadRef.threadId) {
...
export function finalizePromotedDraftThreadsByRef(serverThreadRefs: Iterable<ScopedThreadRef>): void {
	for (const threadRef of serverThreadRefs) {
