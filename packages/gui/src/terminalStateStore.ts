...
import type { ScopedThreadRef, TerminalEvent } from "@t3tools/contracts";
...
function normalizeTerminalGroups(terminalGroups: ThreadTerminalGroup[], terminalIds: string[]): ThreadTerminalGroup[] {
	const validTerminalIdSet = new Set(terminalIds);
...
function setThreadActiveTerminal(state: ThreadTerminalState, terminalId: string): ThreadTerminalState {
	...
	if (normalized.activeTerminalId === terminalId && normalized.activeTerminalGroupId === activeTerminalGroupId) {
		return normalized;
	}
	return {
		...normalized,
...
export function selectTerminalEventEntries(
	...
	return terminalEventEntriesByKey[terminalEventBufferKey(threadRef, terminalId)] ?? EMPTY_TERMINAL_EVENT_ENTRIES;
}
...
interface TerminalStateStoreState {
	...
	setTerminalLaunchContext: (threadRef: ScopedThreadRef, context: ThreadTerminalLaunchContext) => void;
	clearTerminalLaunchContext: (threadRef: ScopedThreadRef) => void;
	setTerminalActivity: (threadRef: ScopedThreadRef, terminalId: string, hasRunningSubprocess: boolean) => void;
		...
		(set) => {
			...
			return {
				...
				setTerminalActivity: (threadRef, terminalId, hasRunningSubprocess) =>
					updateTerminal(threadRef, (state) => setThreadTerminalActivity(state, terminalId, hasRunningSubprocess)),
				...
				clearTerminalState: (threadRef) =>
					set((state) => {
						...
						const hadLaunchContext = state.terminalLaunchContextByThreadKey[threadKey] !== undefined;
						const { [threadKey]: _removed, ...remainingLaunchContexts } = state.terminalLaunchContextByThreadKey;
				...
				removeOrphanedTerminalStates: (activeThreadKeys) =>
					set((state) => {
						...
						const orphanedLaunchContextIds = Object.keys(state.terminalLaunchContextByThreadKey).filter(
							(key) => !activeThreadKeys.has(key),
						);
						const nextTerminalEventEntriesByKey = { ...state.terminalEventEntriesByKey };
						let removedEventEntries = false;
						...
						if (orphanedIds.length === 0 && orphanedLaunchContextIds.length === 0 && !removedEventEntries) {
							return state;
						}
