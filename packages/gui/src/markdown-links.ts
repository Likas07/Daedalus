...
function isRelativePath(path: string): boolean {
	return (
		...
		(!path.startsWith("/") && !WINDOWS_DRIVE_PATH_PATTERN.test(path) && !WINDOWS_UNC_PATH_PATTERN.test(path))
	);
}
...
export function resolveMarkdownFileLinkTarget(href: string | undefined, cwd?: string): string | null {
	if (!href) return null;
...
export function resolveMarkdownFileLinkMeta(href: string | undefined, cwd?: string): MarkdownFileLinkMeta | null {
