...
class MockTransport implements AppServerTransport {
	...
	send(message: unknown): void {
		...
		if (request.method === "shell/snapshot")
			this.respond(request.id, {
				snapshot: { cursor: cursor(1), selectedThreadId: "thread-1", threads: [thread("thread-1")] },
			});
		if (request.method === "thread/snapshot")
			this.respond(request.id, { snapshot: detail(request.params?.threadId ?? "thread-1", 1) });
	}
	...
	emit(method: string, params: unknown): void {
		this.listener?.({ kind: "notification", method, params });
	}
	private respond(id: string | undefined, result: unknown): void {
		this.listener?.({ kind: "response", id, ok: true, result });
	}
}
...
const thread = (threadId: string) => ({
	...
	updatedAt: cursor(1).updatedAt,
	pendingActionCount: 0,
	safetySignals: [],
});
const detail = (threadId: string, seq: number) => ({
	cursor: cursor(seq),
	threadId,
	sessionId: threadId,
	title: threadId,
...
describe("ProjectionRuntime", () => {
	test("hydrates snapshots and suppresses duplicate and wrong-thread events", async () => {
		...
		transport.emit("thread/event", {
			...
			message: { id: "bad", role: "assistant", content: "bad", createdAt: cursor(2).updatedAt },
		} satisfies ThreadDetailEvent);
		transport.emit("thread/event", {
			seq: 2,
			cursor: cursor(2),
			threadId: "thread-1",
			sessionId: "thread-1",
			type: "message-appended",
			message: { id: "a1", role: "assistant", content: "hi", createdAt: cursor(2).updatedAt },
		} satisfies ThreadDetailEvent);
		transport.emit("thread/event", {
			seq: 2,
			cursor: cursor(2),
			...
			sessionId: "thread-1",
			type: "message-appended",
			message: { id: "dup", role: "assistant", content: "dup", createdAt: cursor(2).updatedAt },
		} satisfies ThreadDetailEvent);
	...
	test("cleans detail store on thread switch and preserves runsIn safety signals in shell", async () => {
		...
		transport.emit("shell/event", {
			...
			thread: {
				...thread("thread-2"),
				worktreeId: "wt-1",
				safetySignals: [{ level: "warning", message: "isolated worktree", code: "runsIn" }],
			},
		} satisfies ShellEvent);
	...
	test("preserves selected Thread across reconnect replay and applies only matching newer events", async () => {
		const transport = new MockTransport();
		const runtime = new ProjectionRuntime(new AppServerClient({ transport }), () => {});
		runtime.selectThread("thread-1");
		await tick();
		expect(runtime.state.thread?.threadId).toBe("thread-1");
		runtime.startShell("project-1");
		await tick();
		expect(runtime.state.shell.selectedThreadId).toBe("thread-1");
		transport.emit("thread/event", {
			seq: 2,
			cursor: cursor(2),
			threadId: "thread-1",
			sessionId: "thread-1",
			type: "message-appended",
			message: { id: "u1", role: "user", content: "still selected", createdAt: cursor(2).updatedAt },
		} satisfies ThreadDetailEvent);
		transport.emit("thread/event", {
			seq: 3,
			cursor: cursor(3),
			...
			sessionId: "thread-2",
			type: "message-appended",
			message: { id: "bad", role: "assistant", content: "wrong replay", createdAt: cursor(3).updatedAt },
		} satisfies ThreadDetailEvent);
		expect(runtime.state.thread?.threadId).toBe("thread-1");
		expect(runtime.state.thread?.messages.map((message) => message.content)).toEqual(["still selected"]);
	});
});
