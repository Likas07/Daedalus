...
export type TranscriptRow = Omit<TimelineRow, "kind"> & {
...
export function projectTranscriptEvent(event: AppEvent): TranscriptRow[] {
	...
	if (entries.length > 0)
		return projectSessionEntries(entries, event.sessionId).map((row) => ({
			...row,
			eventId: event.id,
			containsSensitiveRaw: rawLooksSensitive(row.raw),
		}));
	const payload = record(event.payload);
	const streamId = stringField(payload.messageId) || stringField(payload.responseId) || stringField(payload.id);
	const id =
		streamId && isAssistantStream(event.type) ? `stream:${event.sessionId ?? "global"}:${streamId}` : event.id;
	return [
		{
			id,
...
function entriesFromEvent(event: AppEvent): SessionEntryLike[] {
	...
	const candidates = [
		payload.entry,
		payload.entries,
		payload.sessionEntries,
		session.entries,
		payload.message && {
			type: "message",
			id: stringField(payload.messageId) || event.id,
			timestamp: event.ts,
...
function isEntryLike(value: unknown): value is SessionEntryLike {
	return (
		!!value &&
		typeof value === "object" &&
...
function isAssistantStream(type: string): boolean {
	return type.includes("message_delta") || type.includes("message_start") || type.includes("message_end");
}
function isLiveEvent(type: string): boolean {
	return type.includes("delta") || type.includes("output") || type.includes("start");
}
