...
export class ProjectionRuntime {
	...
	startShell(projectId?: string): void {
		...
		let active = true;
		void this.client
			.request("shell/snapshot", { projectId })
			.then((result) => {
				if (!active) return;
				...
				this.state.shell = {
					...shellFromSnapshot(snapshot),
					selectedThreadId: this.#activeThreadId ?? snapshot.selectedThreadId,
					loading: false,
				};
				this.notify();
			})
	...
	selectThread(threadId?: string): void {
		...
		void this.client
			.request("thread/snapshot", { threadId })
			.then((result) => {
				if (!active) return;
			...
			.catch((error) => {
				...
				this.state.thread = {
					...
					rows: [],
					error: error instanceof Error ? error.message : String(error),
					cursor: { seq: 0, updatedAt: new Date(0).toISOString() },
				} as ProjectionThreadStore;
				this.notify();
			});
...
function applyShellEvent(shell: ProjectionShellStore, event: ShellEvent): void {
	if (shell.cursor && event.seq <= shell.cursor.seq) return;
...
function threadFromSnapshot(snapshot: ThreadDetailSnapshot): ProjectionThreadStore {
	return {
		...
		safetySignals: [...snapshot.safetySignals],
		diffIds: [...snapshot.diffIds],
		rows: projectThreadMessages(snapshot),
	};
}

function applyThreadEvent(thread: ProjectionThreadStore, event: ThreadDetailEvent): void {
	if (thread.cursor && event.seq <= thread.cursor.seq) return;
	thread.cursor = event.cursor;
	if (event.message)
		thread.messages = [...thread.messages.filter((message) => message.id !== event.message?.id), event.message];
	if (event.activity)
		thread.activity = [...thread.activity.filter((item) => item.id !== event.activity?.id), event.activity];
