...
export function listSlashCommands(commands: readonly ComposerSlashCommand[]): readonly ComposerSlashCommand[] {
	return commands.map(
		(command) =>
			({
				...command,
				disabled: (command as ComposerSlashCommand & { disabled?: boolean }).disabled,
				disabledReason: (command as ComposerSlashCommand & { disabledReason?: string }).disabledReason,
			}) as ComposerSlashCommand,
	);
}

export async function executeSlashCommand(
	input: string | ComposerSlashCommand,
	...
	if (command === "settings") {
		...
		return { action: "gui", command, message: "Opened settings." };
	}
	if (command === "model" || command === "scoped-models") {
		context.openModelPicker?.();
		return { action: "gui", command, message: "Opened model controls." };
	}
	if (command === "new") {
		context.newSession?.();
		return { action: "gui", command, message: "Started a new session draft." };
	}
	if (command === "reload") {
		await context.reloadResources?.();
		return { action: "runtime", command, message: "Reloaded resources." };
	}
	if (command === "compact") {
		await context.compact?.(context.sessionId);
		return { action: "runtime", command, message: "Compacted session context." };
	}
	if (command === "quit") {
		context.quit?.();
		return { action: "runtime", command, message: "Requested quit." };
	}
	if (GUI_COMMANDS.has(command)) return { action: "gui", command, message: `Handled /${command} in the GUI.` };
	if (RUNTIME_COMMANDS.has(command))
		return { action: "runtime", command, message: `Handled /${command} through the runtime.` };
