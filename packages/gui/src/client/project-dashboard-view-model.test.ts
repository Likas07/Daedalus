...
describe("createProjectDashboardViewModel", () => {
	test("derives branch and diff counts from active diff, not hardcoded placeholders", () => {
		const model = createProjectDashboardViewModel(state({
			activeDiff: {
				branch: "task-21",
				...
				unstagedCount: 1,
				files: [
					{ path: "src/a.ts", status: "modified", staged: true, insertions: 7, deletions: 3, riskGroup: "source" },
					{ path: "test/a.test.ts", status: "added", staged: false, insertions: 11, deletions: 0, riskGroup: "tests" },
				],
	...
	test("renders worktree rows from backend ids and paths", () => {
		const model = createProjectDashboardViewModel(state({
			worktrees: [{ id: "wt-backend", projectId: "project-1", branch: "real-branch", path: "/repo/wt", upstream: null, dirty: true, dirtyCount: 4, activeSessionCount: 2, cleanupRequiresConfirmation: true }],
			projectRoot: "/repo/wt",
		}));
	...
	test("derives sessions, approvals, terminals, and web editor disabled reason", () => {
		const model = createProjectDashboardViewModel(state({
			...
			terminals: [{ id: "t1", terminalId: "t1", cwd: "/repo", cols: 80, rows: 24, status: "running", history: "" }],
		}));
