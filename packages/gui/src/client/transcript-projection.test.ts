...
describe("transcript projection", () => {
	test("projects common app events into safe transcript rows", () => {
		const rows = projectTranscriptEvents([
			{ id: "u1", ts: "2026-04-26T00:00:00.000Z", type: "message/user", sessionId: "s1", payload: { role: "user", content: "hello" } },
			{ id: "t1", ts: "2026-04-26T00:00:01.000Z", type: "tool/call", sessionId: "s1", payload: { command: "bun test", apiKey: "SECRET" } },
			{ id: "a1", ts: "2026-04-26T00:00:02.000Z", type: "approval/requested", sessionId: "s1", payload: { summary: "Approve read" } },
		]);
	...
	test("projects session entry replay payloads", () => {
		const rows = projectTranscriptEvents([{ id: "replay", ts: "now", type: "session/entries", sessionId: "s1", payload: { entries: [{ type: "message", id: "m1", parentId: null, timestamp: "now", message: { role: "assistant", content: [{ type: "text", text: "hello" }] } }] } }]);
	...
	test("updates streaming assistant rows by message id without duplicates", () => {
		const rows = projectTranscriptEvents([
			...
			{ id: "e2", ts: "2", type: "agent/message_delta", sessionId: "s1", payload: { messageId: "m", delta: "hello" } },
		]);
		expect(rows).toHaveLength(1);
