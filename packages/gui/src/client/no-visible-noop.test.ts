...
import { GUI_CAPABILITIES } from "./capability-registry";
...
function runtime(guiState: GuiState): GuiRuntime {
	return {
		...
		async initialize() {},
		async reconnect() {},
		selectSession(sessionId) { guiState.selectedSessionId = sessionId; },
		...
		async createTerminal() { return { terminalId: "term-1", id: "term-1", cwd: "/repo", cols: 100, rows: 24, status: "running", history: "" }; },
...
describe("no visible no-op GUI policy", () => {
	test("every visible command has a handler or an explicit disabled reason", () => {
		const guiState = state({ selectedSessionId: "session-1", events: [{ id: "event-1", ts: "now", type: "turn/running", sessionId: "session-1", payload: { turnId: "turn-1" } }] });
		const commands = createCommandRegistry({ guiState, ui: uiState(), runtime: runtime(guiState), focusComposer() {}, exportDiagnostics() {} });
	...
	test("partial or disabled capabilities always explain why mutation controls are unavailable", () => {
		const incomplete = GUI_CAPABILITIES.filter((capability) => capability.status !== "wired");
		expect(incomplete.length).toBeGreaterThan(0);
		expect(incomplete.filter((capability) => !capability.disabledReason?.trim())).toEqual([]);
	});
});
