...
import { type GuiCapabilityId, getGuiCapability } from "./capability-registry";
import { type CommandDescriptor, type CommandId, commandDisabledReason, GUI_COMMANDS } from "./commands";
import {
	extensionCommands,
	type RendererSafeExtensionMetadata,
	type RendererSafeExtensionSurface,
...
export function createCommandRegistry(context: CommandRegistryContext): readonly RegisteredCommand[] {
	...
	const extensionRegistryCommands = extensionCommands(context.extensions ?? []).map(
		(command): RegisteredCommand => ({
			...
			disabledReason: context.dispatchExtensionCommand
				? undefined
				: "Extension command dispatch is not available in this renderer.",
		}),
	);
...
function installNativeCommandRegistryBridge(
	...
	globalThis.addEventListener("daedalus:native-command", (event) => {
		...
		const commandId =
			detail?.id === "toggle-terminal"
				? "toggle-terminal"
				: detail?.id === "export-diagnostics"
					? "export-diagnostics"
					: undefined;
		if (!commandId) return;
...
function getDisabledReason(command: CommandDescriptor, context: CommandRegistryContext): string | undefined {
	...
	if (command.id === "cancel-turn" && !findActiveTurnId(context.guiState.events, context.guiState.selectedSessionId))
		return "No active turn is running in the selected session.";
	if (command.id === "stop-session" && !context.guiState.selectedSessionId) return "No session is selected.";
	if (command.id === "open-model-selector" && context.guiState.models.length === 0)
		return "No models are available from the server yet.";
	return undefined;
}
...
function runHandler(commandId: CommandId, context: CommandRegistryContext): (() => void | Promise<void>) | undefined {
	switch (commandId) {
		...
		case "open-project":
			return () => {
				context.ui.paletteMode = "project";
				context.ui.paletteOpen = true;
			};
		case "open-settings":
		...
		case "open-model-selector":
			return () => {
				context.ui.popoverKind = "model";
			};
		case "export-diagnostics":
			return () => {
				void context.exportDiagnostics?.(context.runtime.exportDiagnostics());
			};
		case "toggle-terminal":
			return () => {
				context.ui.terminalOpen = !context.ui.terminalOpen;
			};
		case "toggle-inspector":
			return () => {
				context.ui.rightOpen = !context.ui.rightOpen;
			};
		case "show-approval-queue":
			return () => document.querySelector<HTMLElement>('[data-testid="approval-queue"]')?.scrollIntoView();
		case "cancel-turn":
			return () => {
				const turnId = findActiveTurnId(context.guiState.events, context.guiState.selectedSessionId);
				if (context.guiState.selectedSessionId && turnId)
					void context.runtime.cancelTurn(context.guiState.selectedSessionId, turnId);
			};
		case "stop-session":
			return () => {
				if (context.guiState.selectedSessionId)
					void context.runtime.stopSession(context.guiState.selectedSessionId);
			};
		case "create-terminal":
			return () => {
				void context.runtime.createTerminal({
					cwd: context.guiState.projectRoot ?? "/",
					projectId: context.guiState.lastProjectId,
					cols: 100,
					rows: 24,
				});
				context.ui.terminalOpen = true;
			};
		case "set-access-mode-supervised":
			return () => void context.runtime.setAccessMode("supervised");
		case "set-access-mode-auto-accept":
			return () => void context.runtime.setAccessMode("auto-accept" as AccessMode);
		case "set-access-mode-unrestricted":
			return () => void context.runtime.setAccessMode("unrestricted");
