...
describe("thread selection memoization", () => {
	...
	it("checks thread existence without materializing the full thread", () => {
		...
		expect(selectThreadExistsByRef(state, scopeThreadRef(thread.environmentId, ThreadId.make("missing")))).toBe(
			false,
		);
...
describe("setThreadBranch", () => {
	it("updates only the scoped thread environment", () => {
		...
		expect(environmentStateOf(next, localEnvironmentId).threadShellById[sharedThreadId]?.branch).toBe("local-branch");
		expect(environmentStateOf(next, remoteEnvironmentId).threadShellById[sharedThreadId]?.branch).toBe("remote-next");
		expect(environmentStateOf(next, remoteEnvironmentId).threadShellById[sharedThreadId]?.worktreePath).toBe(
			"/tmp/remote-worktree",
		);
	});
});
...
describe("incremental orchestration updates", () => {
	...
	it("reuses an existing project row when project.created arrives with a new id for the same cwd", () => {
		...
		expect(localEnvironmentStateOf(next).projectById[recreatedProjectId]?.id).toBe(recreatedProjectId);
	});
	...
	it("removes stale project index entries when thread.created recreates a thread under a new project", () => {
		...
		expect(localEnvironmentStateOf(next).threadIdsByProjectId[recreatedProjectId]).toEqual([threadId]);
	});
	...
	it("rebinds live turn diffs to the authoritative assistant message when it arrives later", () => {
		...
		expect(threadsOf(next)[0]?.turnDiffSummaries[0]?.assistantMessageId).toBe(MessageId.make("assistant-real"));
		expect(threadsOf(next)[0]?.latestTurn?.assistantMessageId).toBe(MessageId.make("assistant-real"));
	});
	...
	it("reverts messages, plans, activities, and checkpoints by retained turns", () => {
		...
		expect(threadsOf(next)[0]?.messages.map((message) => message.id)).toEqual(["user-1", "assistant-1"]);
		expect(threadsOf(next)[0]?.proposedPlans.map((plan) => plan.id)).toEqual(["plan-1"]);
		expect(threadsOf(next)[0]?.activities.map((activity) => activity.id)).toEqual([EventId.make("activity-1")]);
		expect(threadsOf(next)[0]?.turnDiffSummaries.map((summary) => summary.turnId)).toEqual([TurnId.make("turn-1")]);
	});
