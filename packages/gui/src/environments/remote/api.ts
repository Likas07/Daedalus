...
async function readRemoteAuthErrorMessage(response: Response, fallbackMessage: string): Promise<string> {
...
async function fetchRemoteJson<T>(input: {
	...
	try {
		...
		throw new Error(`Failed to fetch remote auth endpoint ${requestUrl} (${(error as Error).message}).`, {
			cause: error,
		});
	}
