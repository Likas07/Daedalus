import { EnvironmentId, type OrchestrationShellSnapshot, ProjectId, ThreadId, TurnId } from "@t3tools/contracts";
...
function makeThreadShellSnapshot(params: {
	...
	readonly sessionStatus?: "idle" | "starting" | "running" | "ready" | "interrupted" | "stopped" | "error";
...
describe("retainThreadDetailSubscription", () => {
	...
	it("keeps thread detail subscriptions warm across releases until idle eviction", async () => {
		const { retainThreadDetailSubscription, startEnvironmentConnectionService, resetEnvironmentServiceForTests } =
	...
	it("allows a larger idle cache before capacity eviction starts", async () => {
		...
		for (let index = 0; index < 12; index += 1) {
			const release = retainThreadDetailSubscription(environmentId, ThreadId.make(`thread-${index + 1}`));
			release();
		}
