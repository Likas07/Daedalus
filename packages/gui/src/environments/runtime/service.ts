...
function toAppliedProjectionVersion(snapshot: Pick<OrchestrationShellSnapshot, "snapshotSequence" | "updatedAt">): {
	readonly sequence: number;
...
function markAppliedProjectionSnapshot(
	...
	if (currentVersion !== null && compareAppliedProjectionVersion(currentVersion, nextVersion) >= 0) {
		return;
	}
...
function clearThreadDetailSubscriptionEviction(entry: ThreadDetailSubscriptionEntry): ThreadDetailSubscriptionEntry {
...
function isNonIdleThreadDetailSubscription(entry: ThreadDetailSubscriptionEntry): boolean {
	...
	return (
		Boolean(orchestrationStatus && orchestrationStatus !== "idle" && orchestrationStatus !== "stopped") ||
...
function attachThreadDetailSubscription(entry: ThreadDetailSubscriptionEntry): boolean {
	...
	entry.unsubscribe = connection.client.orchestration.subscribeThread({ threadId: entry.threadId }, (item) => {
...
function scheduleThreadDetailSubscriptionEviction(entry: ThreadDetailSubscriptionEntry): void {
	...
	entry.evictionTimeoutId = setTimeout(() => {
		...
		disposeThreadDetailSubscriptionByKey(getThreadDetailSubscriptionKey(entry.environmentId, entry.threadId));
...
function reconcileThreadDetailSubscriptionEvictionState(entry: ThreadDetailSubscriptionEntry): void {
	clearThreadDetailSubscriptionEviction(entry);
	if (!shouldEvictThreadDetailSubscription(entry)) {
		return;
	}
...
function reconcileThreadDetailSubscriptionEvictionForThread(environmentId: EnvironmentId, threadId: ThreadId): void {
	const entry = threadDetailSubscriptions.get(getThreadDetailSubscriptionKey(environmentId, threadId));
	if (!entry) {
		return;
	}
...
function reconcileThreadDetailSubscriptionEvictionForEnvironment(environmentId: EnvironmentId): void {
...
export function retainThreadDetailSubscription(environmentId: EnvironmentId, threadId: ThreadId): () => void {
...
function coalesceOrchestrationUiEvents(events: ReadonlyArray<OrchestrationEvent>): OrchestrationEvent[] {
	if (events.length < 2) {
		return [...events];
	}
...
function syncThreadUiFromStore() {
	...
	markPromotedDraftThreadsByRef(threads.map((thread) => scopeThreadRef(thread.environmentId, thread.id)));
}
...
function applyRecoveredEventBatch(events: ReadonlyArray<OrchestrationEvent>, environmentId: EnvironmentId) {
	if (events.length === 0) {
		return;
	}
	...
	const needsProjectUiSync = events.some(
		(event) =>
			event.type === "project.created" || event.type === "project.meta-updated" || event.type === "project.deleted",
	);
	...
	const needsThreadUiSync = events.some((event) => event.type === "thread.created" || event.type === "thread.deleted");
	...
	for (const threadId of batchEffects.clearDeletedThreadIds) {
		...
		useUiStateStore.getState().clearThreadUi(scopedThreadKey(scopeThreadRef(environmentId, threadId)));
	}
	for (const event of events) {
...
export function applyEnvironmentThreadDetailEvent(event: OrchestrationEvent, environmentId: EnvironmentId) {
...
function applyShellEvent(event: OrchestrationShellStreamEvent, environmentId: EnvironmentId) {
	...
	const threadId =
		event.kind === "thread-upserted" ? event.thread.id : event.kind === "thread-removed" ? event.threadId : null;
...
function createEnvironmentConnectionHandlers() {
	return {
		...
		applyTerminalEvent: (event: TerminalEvent, environmentId: EnvironmentId) => {
			...
			const hasDraftThread = useComposerDraftStore.getState().getDraftThreadByRef(threadRef) !== null;
			if (
...
function createPrimaryEnvironmentClient(knownEnvironment: ReturnType<typeof getPrimaryKnownEnvironment>) {
	const wsBaseUrl = getKnownEnvironmentWsBaseUrl(knownEnvironment);
	if (!wsBaseUrl) {
		throw new Error(`Unable to resolve websocket URL for ${knownEnvironment?.label ?? "primary environment"}.`);
	}

	return createWsRpcClient(new WsTransport(wsBaseUrl));
}

function createSavedEnvironmentClient(record: SavedEnvironmentRecord, bearerToken: string): WsRpcClient {
...
async function ensureSavedEnvironmentConnection(
	...
	const bearerToken = options?.bearerToken ?? (await readSavedEnvironmentBearerToken(record.environmentId));
	if (!bearerToken) {
		useSavedEnvironmentRuntimeStore.getState().patch(record.environmentId, {
...
async function syncSavedEnvironmentConnections(records: ReadonlyArray<SavedEnvironmentRecord>): Promise<void> {
	...
	await Promise.all(staleEnvironmentIds.map((environmentId) => disconnectSavedEnvironment(environmentId)));
	await Promise.all(records.map((record) => ensureSavedEnvironmentConnection(record).catch(() => undefined)));
}
...
export function readEnvironmentConnection(environmentId: EnvironmentId): EnvironmentConnection | null {
...
export async function addSavedEnvironment(input: {
	...
	const didPersistBearerToken = await writeSavedEnvironmentBearerToken(environmentId, bearerSession.sessionToken);
...
export async function ensureEnvironmentConnectionBootstrapped(environmentId: EnvironmentId): Promise<void> {
...
export async function resetEnvironmentServiceForTests(): Promise<void> {
	...
	await Promise.all([...environmentConnections.keys()].map((environmentId) => removeConnection(environmentId)));
}
