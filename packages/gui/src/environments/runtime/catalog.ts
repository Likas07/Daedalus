...
function toPersistedSavedEnvironmentRecord(record: SavedEnvironmentRecord): PersistedSavedEnvironmentRecord {
	return {
...
function persistSavedEnvironmentRegistryState(byId: Record<EnvironmentId, SavedEnvironmentRecord>): void {
	try {
		void ensureLocalApi()
			.persistence.setSavedEnvironmentRegistry(
				valuesOfSavedEnvironmentRegistry(byId).map((record) => toPersistedSavedEnvironmentRecord(record)),
			)
			.catch((error) => {
...
function replaceSavedEnvironmentRegistryState(records: ReadonlyArray<SavedEnvironmentRecord>): void {
...
export function getSavedEnvironmentRecord(environmentId: EnvironmentId): SavedEnvironmentRecord | null {
...
export async function readSavedEnvironmentBearerToken(environmentId: EnvironmentId): Promise<string | null> {
...
export async function removeSavedEnvironmentBearerToken(environmentId: EnvironmentId): Promise<void> {
...
interface SavedEnvironmentRuntimeStoreState {
	...
	readonly patch: (environmentId: EnvironmentId, patch: Partial<SavedEnvironmentRuntimeState>) => void;
...
export const useSavedEnvironmentRuntimeStore = create<SavedEnvironmentRuntimeStoreState>()((set) => ({
	byId: {},
...
export function getSavedEnvironmentRuntimeState(environmentId: EnvironmentId): SavedEnvironmentRuntimeState {
	return useSavedEnvironmentRuntimeStore.getState().byId[environmentId] ?? DEFAULT_SAVED_ENVIRONMENT_RUNTIME_STATE;
}
