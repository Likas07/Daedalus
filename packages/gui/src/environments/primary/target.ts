...
function swapBaseUrlProtocol(rawValue: string, nextProtocol: "http:" | "https:" | "ws:" | "wss:"): string {
...
function resolveDesktopPrimaryTarget(): PrimaryEnvironmentTarget | null {
	...
	if (!desktopBootstrap.httpBaseUrl || !desktopBootstrap.wsBaseUrl) {
		throw new Error("Desktop bootstrap must provide both httpBaseUrl and wsBaseUrl for the local environment.");
	}
...
export function resolvePrimaryEnvironmentHttpUrl(pathname: string, searchParams?: Record<string, string>): string {
...
export function readPrimaryEnvironmentTarget(): PrimaryEnvironmentTarget | null {
	return resolveDesktopPrimaryTarget() ?? resolveConfiguredPrimaryTarget() ?? resolveWindowOriginPrimaryTarget();
}
