...
function makeProject(overrides: Partial<Project> & Pick<Project, "id" | "environmentId" | "name">): Project {
	return {
...
describe("environment grouping", () => {
	...
	describe("logical project grouping for sidebar", () => {
		it("computes correct logical key for grouped projects and aggregates threads", () => {
			...
			expect(sharedGroup!.map((p) => p.environmentId).toSorted()).toEqual([primaryEnvId, remoteEnvId].toSorted());
		...
		it("local-only and remote-only projects remain ungrouped", () => {
			...
			// Local-only group
			const localKey = deriveLogicalProjectKey(allProjects.find((p) => p.id === localOnlyProjectId)!);
			expect(groups.get(localKey)).toHaveLength(1);

			// Remote-only group
			const remoteKey = deriveLogicalProjectKey(allProjects.find((p) => p.id === remoteOnlyProjectId)!);
