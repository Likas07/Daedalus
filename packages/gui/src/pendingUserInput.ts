...
export function setPendingUserInputCustomAnswer(
	...
	const selectedOptionLabels =
		customAnswer.trim().length > 0 ? undefined : normalizeSelectedOptionLabels(draft?.selectedOptionLabels);
...
export function togglePendingUserInputOptionSelection(
	...
	if (question.multiSelect) {
		...
		return {
			customAnswer: "",
			...(nextSelectedOptionLabels.length > 0 ? { selectedOptionLabels: nextSelectedOptionLabels } : {}),
		};
	}
...
export function derivePendingUserInputProgress(
	...
	const resolvedAnswer = activeQuestion ? resolvePendingUserInputAnswer(activeQuestion, activeDraft) : null;
