...
describe("desktop update UI helpers", () => {
	...
	it("includes the downloaded version in the install confirmation copy", () => {
		...
		).toContain("Install update 1.1.1 and restart Daedalus?");
	});
	...
	it("falls back to generic install confirmation copy when no version is available", () => {
		...
		).toContain("Install update and restart Daedalus?");
	});
});
...
describe("canCheckForUpdate", () => {
	...
	it("returns false while downloading", () => {
		expect(canCheckForUpdate({ ...baseState, status: "downloading", downloadPercent: 50 })).toBe(false);
	});
	...
	it("returns true when an update is available", () => {
		expect(canCheckForUpdate({ ...baseState, status: "available", availableVersion: "1.1.0" })).toBe(true);
	});
