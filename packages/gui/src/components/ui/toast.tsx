...
import { buildVisibleToastLayout, shouldHideCollapsedToastContent, shouldRenderThreadScopedToast } from "./toast.logic";
...
export type ThreadToastData = {
	...
	actionVariant?: "default" | "destructive" | "destructive-outline" | "ghost" | "link" | "outline" | "secondary";
};
...
const toastExpandablePanelClassName = "mt-2 max-h-40 min-h-0 overflow-y-auto overscroll-contain pr-0.5 select-text";
...
function deriveToastBodyDescriptor(toast: {
	...
	const stackedActionLayout = toast.actionProps !== undefined && toast.data?.actionLayout === "stacked-end";
	const actionVariant: NonNullable<ThreadToastData["actionVariant"]> = toast.data?.actionVariant ?? "default";
	const copyErrorText =
...
function ToastBodyContent({
	...
	return (
		<>
			...
			{hasTrailingControls ? (
				<div className={cn("flex items-center gap-1.5", stackedActionLayout ? "w-full justify-end" : "shrink-0")}>
...
function Toasts({ position = "top-right" }: { position: ToastPosition }) {
	...
	const visibleToasts = toasts.filter((toast) => shouldRenderThreadScopedToast(toast.data, activeThreadRef));
					...
					return (
						<Toast.Root
							className={cn(
								...
								visibleIndex > 0 ? "not-data-expanded:h-(--toast-calc-height) data-expanded:h-auto" : "h-auto",
