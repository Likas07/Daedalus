import type { FilesystemBrowseEntry, KeybindingCommand } from "@t3tools/contracts";
...
export function buildThreadActionItems<TThread extends BuildThreadActionItemsThread>(input: {
	...
	const visibleThreads = input.limit === undefined ? sortedThreads : sortedThreads.slice(0, input.limit);
		...
		return Object.assign(
			{
				...
				timestamp: formatRelativeTimeLabel(thread.latestUserMessageAt ?? thread.updatedAt ?? thread.createdAt),
				icon: input.icon,
			},
...
export function filterCommandPaletteGroups(input: {
	...
	return searchableGroups.flatMap((group) => {
		const items = group.items
			...
			.filter(
				(entry): entry is { item: (typeof group.items)[number]; index: number; rank: number } => entry !== null,
			)
