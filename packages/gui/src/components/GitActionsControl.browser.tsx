...
vi.mock("~/composerDraftStore", async () => {
	...
	return {
		...
		useComposerDraftStore: Object.assign((selector: (state: unknown) => unknown) => selector(draftStoreState), {
...
vi.mock("~/store", () => ({
	selectEnvironmentState: (state: { environmentStateById: Record<string, unknown> }, environmentId: string | null) => {
		if (!environmentId) {
...
function Harness() {
	const [activeThreadRef, setActiveThreadRef] = useState(scopeThreadRef(ENVIRONMENT_A, SHARED_THREAD_ID));

	return (
		<>
			<button type="button" onClick={() => setActiveThreadRef(scopeThreadRef(ENVIRONMENT_B, SHARED_THREAD_ID))}>
				Switch environment
			</button>
...
describe("GitActionsControl thread-scoped progress toast", () => {
	...
	it("keeps an in-flight git action toast pinned to the thread ref that started it", async () => {
		...
		try {
			...
			expect(switchEnvironmentButton, 'Unable to find button containing "Switch environment"').toBeTruthy();
	...
	it("debounces focus-driven git status refreshes", async () => {
		...
		const screen = await render(
			<GitActionsControl gitCwd={GIT_CWD} activeThreadRef={scopeThreadRef(ENVIRONMENT_A, SHARED_THREAD_ID)} />,
			{
				container: host,
			},
		);
	...
	it("syncs the live branch into the active draft thread when no server thread exists", async () => {
		...
		try {
			...
			expect(setDraftThreadContextSpy).toHaveBeenCalledWith(scopeThreadRef(ENVIRONMENT_A, SHARED_THREAD_ID), {
				branch: BRANCH_NAME,
				worktreePath: null,
			});
