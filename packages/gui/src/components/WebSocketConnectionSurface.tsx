...
function buildReconnectTitle(_status: WsConnectionStatus): string {
	return "Disconnected from Daedalus app server";
}

function describeRecoveredToast(previousDisconnectedAt: string | null, connectedAt: string | null): string {
...
function SlowRpcAckRequestDetails({ requests }: { requests: ReadonlyArray<SlowRpcAckRequest> }) {
	return (
		<ul className="space-y-2.5 text-xs text-muted-foreground">
			{requests.map((req) => (
				<li className="min-w-0 border-border/50 border-b pb-2 last:border-b-0 last:pb-0" key={req.requestId}>
					<div className="wrap-break-word font-medium text-foreground">{req.tag}</div>
					<div className="mt-0.5 font-mono text-[10px] leading-snug opacity-90">{req.requestId}</div>
...
export function shouldAutoReconnect(status: WsConnectionStatus, trigger: WsAutoReconnectTrigger): boolean {
	...
	return status.online && status.hasConnected && (uiState === "reconnecting" || status.reconnectPhase === "exhausted");
}

export function shouldRestartStalledReconnect(status: WsConnectionStatus, expectedNextRetryAt: string): boolean {
	return (
...
export function WebSocketConnectionCoordinator() {
	...
	}, [status.hasConnected, status.nextRetryAt, status.online, status.reconnectAttemptCount, status.reconnectPhase]);
	...
	useEffect(() => {
			...
			const toastPayload = shouldShowOfflineToast
				...
				: shouldShowExhaustedToast
					? stackedThreadToast({
							...
							timeout: 0,
							title: "Disconnected from Daedalus app server",
							type: "error",
						})
					: stackedThreadToast({
							actionProps: {
		...
		if (
			...
			const successToast = {
				...
				title: "Reconnected to Daedalus app server",
