...
export function BranchToolbarBranchSelector({
	...
	// ---------------------------------------------------------------------------
	const threadRef = useMemo(() => scopeThreadRef(environmentId, threadId), [environmentId, threadId]);
	...
	const activeProjectRef = serverThread
		...
		: draftThread
			...
			: null;
	const activeProjectSelector = useMemo(() => createProjectSelectorByRef(activeProjectRef), [activeProjectRef]);
	...
	const createBranchItemValue = canCreateBranch ? `__create_new_branch__:${trimmedBranchQuery}` : null;
	...
	const filteredBranchPickerItems = useMemo(
		...
		[branchPickerItems, checkoutPullRequestItemValue, createBranchItemValue, normalizedDeferredBranchQuery],
	);
	...
	const selectBranch = (branch: GitBranch) => {
		...
		runBranchAction(async () => {
			...
			try {
				...
				const nextBranchName = branch.isRemote ? (checkoutResult.branch ?? selectedBranchName) : selectedBranchName;
	...
	function renderPickerItem(itemValue: string, index: number) {
		...
		const hasSecondaryWorktree = branch.worktreePath && activeProjectCwd && branch.worktreePath !== activeProjectCwd;
