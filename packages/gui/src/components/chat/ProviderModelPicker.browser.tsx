import { EnvironmentId, type ProviderKind, type ServerProvider } from "@t3tools/contracts";
...
describe("ProviderModelPicker", () => {
	...
	it("shows favorites first in the provider sidebar", async () => {
		...
		try {
			...
			await vi.waitFor(() => {
				expect(getSidebarProviderOrder().slice(0, 3)).toEqual(["favorites", "codex", "claudeAgent"]);
			});
		} finally {
			await mounted.cleanup();
		}
	...
	it("shows locked provider header and only its models in locked mode", async () => {
		...
		try {
			...
			await vi.waitFor(() => {
				...
				expect(text).toContain("Claude");
				expect(getVisibleModelNames()).toEqual(["Claude Sonnet 4.6", "Claude Opus 4.6", "Claude Haiku 4.5"]);
			});
		} finally {
	...
	it("falls back to the active provider's first model when props.model belongs to another provider (#1982)", async () => {
		...
		try {
			const trigger = document.querySelector<HTMLElement>('[data-chat-provider-model-picker="true"]');
	...
	it("closes the picker when escape is pressed in search", async () => {
		...
		try {
			...
			const searchInputElement = document.querySelector<HTMLInputElement>('input[placeholder="Search models..."]');
	...
	it("toggles favorite stars when clicked", async () => {
		...
		try {
			...
			expect(initialAriaLabel === "Add to favorites" || initialAriaLabel === "Remove from favorites").toBe(true);
