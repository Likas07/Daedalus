...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
	...
	const keybindings = useMemo<ResolvedKeybindingsConfig>(() => providedKeybindings ?? [], [providedKeybindings]);
	...
	const favoriteOrder = useMemo(() => {
		return new Map(favorites.map((favorite, index) => [`${favorite.provider}:${favorite.model}`, index]));
	}, [favorites]);
	...
	const readyProviderSet = useMemo(() => {
		...
		return new Set(
			props.providers.filter((provider) => provider.status === "ready").map((provider) => provider.provider),
		);
	}, [props.providers]);
	...
	}, [favoriteOrder, favoritesSet, flatModels, props.lockedProvider, searchQuery, selectedProvider]);

	const handleModelSelect = useCallback(
		(modelSlug: string, provider: ProviderKind) => {
			const resolvedModel = resolveSelectableModel(provider, modelSlug, modelOptionsByProvider[provider]);
			if (resolvedModel) {
	...
	const LockedProviderIcon = isLocked && props.lockedProvider ? PROVIDER_ICON_BY_PROVIDER[props.lockedProvider] : null;
	const modelJumpCommandByKey = useMemo(() => {
		const mapping = new Map<string, NonNullable<ReturnType<typeof modelPickerJumpCommandForIndex>>>();
	...
	const modelJumpModelKeys = useMemo(() => [...modelJumpCommandByKey.keys()], [modelJumpCommandByKey]);
			...
			>
				...
				{isLocked && LockedProviderIcon && props.lockedProvider && (
					<div className="flex items-center gap-2 px-4 py-3 border-b">
						...
						<span className="font-medium text-sm">{PROVIDER_DISPLAY_NAMES[props.lockedProvider]}</span>
					</div>
				)}
