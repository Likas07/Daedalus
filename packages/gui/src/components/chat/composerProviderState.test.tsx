...
function modelWith(descriptors: ReadonlyArray<ProviderOptionDescriptor>): ReadonlyArray<ServerProviderModel> {
	return [{ slug: MODEL, name: MODEL, isCustom: false, capabilities: { optionDescriptors: descriptors } }];
}

function selections(...entries: Array<[string, string | boolean]>): ReadonlyArray<ProviderOptionSelection> {
...
describe("getComposerProviderState", () => {
	...
	it("preserves selections that match defaults so deepMerge can overwrite prior state", () => {
		...
		expect(state.modelOptionsForDispatch).toEqual(selections(["effort", "high"], ["fastMode", false]));
	});
	...
	it("does not add ultrathink class names when the descriptor has no promptInjectedValues", () => {
		const state = getComposerProviderState({
			provider: PROVIDER,
			model: MODEL,
			models: modelWith([selectDescriptor("effort", [{ id: "high", label: "High", isDefault: true }])]),
			prompt: "Ultrathink:\nInvestigate this failure",
			modelOptions: undefined,
		});
...
describe("provider traits render guards", () => {
	it("returns null when no thread target is provided", () => {
		const models = modelWith([selectDescriptor("effort", [{ id: "high", label: "High", isDefault: true }])]);
		const args = {
			provider: PROVIDER,
			model: MODEL,
			models,
