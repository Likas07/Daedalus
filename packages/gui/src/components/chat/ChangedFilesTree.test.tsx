...
describe("ChangedFilesTree", () => {
	...
	it.each([
		...
		},
		{
			...
			visibleLabels: ["apps/server/src", "git/Layers", "provider/Layers", "GitCore.ts", "CodexAdapter.ts"],
		},
		{
			name: "mixed root files and nested compacted directories",
			files: [
			...
			visibleLabels: ["README.md", "packages", "shared/src", "contracts/src", "git.ts", "orchestration.ts"],
		},
	])("renders $name expanded on the first render when expand-all is active", ({ files, visibleLabels }) => {
		const markup = renderToStaticMarkup(
			<ChangedFilesTree
				turnId={TurnId.make("turn-1")}
				files={files}
