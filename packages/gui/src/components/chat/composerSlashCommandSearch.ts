import { insertRankedSearchResult, normalizeSearchQuery, scoreQueryMatch } from "@t3tools/shared/searchRanking";
...
function scoreSlashCommandItem(
	...
	const primaryValue = item.type === "slash-command" ? item.command.toLowerCase() : item.command.name.toLowerCase();
...
export function searchSlashCommandItems(
	items: ReadonlyArray<Extract<ComposerCommandItem, { type: "slash-command" | "provider-slash-command" }>>,
	query: string,
