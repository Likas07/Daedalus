...
import { createContext, memo, type ReactNode, use, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
import { type deriveTimelineEntries, formatElapsed } from "../../session-logic";
...
function TimelineRowContent({ row }: { row: TimelineRow }) {
	...
	return (
		<div
			className={cn("pb-4", row.kind === "message" && row.message.role === "assistant" ? "group/assistant" : null)}
...
function LiveMessageMeta({
	...
	const elapsed = durationStart ? formatElapsed(durationStart, new Date(nowMs).toISOString()) : null;
...
const UserMessageTerminalContextInlineLabel = memo(function UserMessageTerminalContextInlineLabel(props: {
	context: ParsedTerminalContextEntry;
}) {
	const tooltipText =
		props.context.body.length > 0 ? `${props.context.header}\n${props.context.body}` : props.context.header;
...
const UserMessageBody = memo(function UserMessageBody(props: {
		...
		if (hasEmbeddedInlineLabels) {
			...
			if (inlineNodes.length > 0) {
				if (cursor < props.text.length) {
					inlineNodes.push(
						<span key={`user-message-terminal-context-inline-rest:${cursor}`}>{props.text.slice(cursor)}</span>,
					);
				}
	...
	return (
		<div className="whitespace-pre-wrap wrap-break-word text-sm leading-relaxed text-foreground">{props.text}</div>
	);
});
...
function workEntryRawCommand(workEntry: Pick<TimelineWorkEntry, "command" | "rawCommand">): string | null {
...
const SimpleWorkEntryRow = memo(function SimpleWorkEntryRow(props: {
	...
	const preview =
		rawPreview &&
		normalizeCompactToolLabel(rawPreview).toLowerCase() === normalizeCompactToolLabel(heading).toLowerCase()
			? null
			: rawPreview;
