...
import { DEFAULT_MODEL_BY_PROVIDER, EnvironmentId, type ModelSelection, ThreadId } from "@t3tools/contracts";
import "../../index.css";
...
describe("CompactComposerControlsMenu", () => {
	...
	it("shows a Claude thinking on/off section for Haiku", async () => {
		await using _ = await mountMenu({
			modelSelection: createModelSelection("claudeAgent", "claude-haiku-4-5", [{ id: "thinking", value: true }]),
		});
	...
	it("shows prompt-controlled Ultrathink state with selectable effort controls", async () => {
		await using _ = await mountMenu({
			modelSelection: createModelSelection("claudeAgent", "claude-opus-4-6", [{ id: "effort", value: "high" }]),
			prompt: "Ultrathink:\nInvestigate this",
		});
	...
	it("warns when ultrathink appears in prompt body text", async () => {
		...
		await vi.waitFor(() => {
			const text = document.body.textContent ?? "";
			expect(text).toContain('Your prompt contains "ultrathink" in the text. Remove it to change this option.');
		});
	});
