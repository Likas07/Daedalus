import type { ProjectEntry, ProviderKind, ServerProviderSkill, ServerProviderSlashCommand } from "@t3tools/contracts";
							...
							<CommandGroup>
								...
								<p className="text-muted-foreground/70 text-xs">
									{props.isLoading
										...
										: (props.emptyStateText ?? "No skills found. Try / to browse provider commands.")}
								</p>
							</CommandGroup>
						) : (
							<p className="text-muted-foreground/70 text-xs">
...
const ComposerCommandMenuItem = memo(function ComposerCommandMenuItem(props: {
	...
	const skillSourceLabel = props.item.type === "skill" ? formatProviderSkillInstallSource(props.item.skill) : null;
	...
	return (
		...
		>
			{props.item.type === "path" ? (
				<VscodeEntryIcon pathValue={props.item.path} kind={props.item.pathKind} theme={props.resolvedTheme} />
			) : null}
			{props.item.type === "slash-command" ? <BotIcon className="size-4 shrink-0 text-muted-foreground/80" /> : null}
			...
			<span className="flex min-w-0 flex-1 items-center gap-2">
				...
				<span className="min-w-0 flex-1 truncate text-muted-foreground/70 text-xs">{props.item.description}</span>
			</span>
			{skillSourceLabel ? (
