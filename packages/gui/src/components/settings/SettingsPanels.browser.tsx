...
vi.mock("../../environments/runtime", () => {
	...
	return {
		...
		useSavedEnvironmentRegistryStore: (selector: (state: { byId: Record<string, never> }) => unknown) =>
			selector({ byId: {} }),
		useSavedEnvironmentRuntimeStore: (selector: (state: { byId: Record<string, never> }) => unknown) =>
			selector({ byId: {} }),
	};
});
...
function makeClientSession(input: {
	...
	return {
		...
		expiresAt: makeUtc(input.expiresAt),
		lastConnectedAt:
			input.lastConnectedAt === undefined || input.lastConnectedAt === null ? null : makeUtc(input.lastConnectedAt),
	};
}
...
const createDesktopBridgeStub = (overrides?: {
	...
	return {
		...
		downloadUpdate: vi.fn().mockResolvedValue({ accepted: false, completed: false, state: idleUpdateState }),
		installUpdate: vi.fn().mockResolvedValue({ accepted: false, completed: false, state: idleUpdateState }),
		onUpdateState: () => () => {},
	};
};
...
describe("GeneralSettingsPanel observability", () => {
	...
	it("hides owner pairing tools in browser-served loopback builds without remote exposure", async () => {
		...
		await expect.element(page.getByRole("heading", { name: "Remote environments", exact: true })).toBeInTheDocument();
	});
	...
	it("shows diagnostics inside About with a single logs-folder action", async () => {
		...
		await expect.element(page.getByText("/repo/project/.t3/logs", { exact: true })).toBeInTheDocument();
		await expect
	...
	it("creates and shows a pairing link when network access is enabled", async () => {
		...
		await expect.element(page.getByText("Client · Mobile · iOS · Safari · 192.168.1.88")).toBeInTheDocument();
		await expect.element(page.getByRole("button", { name: /^(Copy|Show link)$/ })).toBeInTheDocument();
		await expect.element(page.getByText("Revoke others")).toBeInTheDocument();
	});
	...
	it("shows a disabled network access toggle with guidance in desktop builds", async () => {
		...
		await expect
			.element(page.getByText("Daedalus will restart to expose this environment over the network."))
			.toBeInTheDocument();
		...
		await expect.element(page.getByText("Reachable at http://192.168.1.44:3773")).toBeInTheDocument();
	});
