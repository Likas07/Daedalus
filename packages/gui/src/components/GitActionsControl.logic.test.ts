...
describe("when: on default branch without open PR", () => {
	...
	it("resolveQuickAction returns push when branch is ahead", () => {
		const quick = resolveQuickAction(status({ branch: "main", aheadCount: 2, pr: null }), false, true);
		assert.deepInclude(quick, {
			kind: "run_action",
			action: "commit_push",
			label: "Push",
...
describe("when: branch has no upstream configured", () => {
	...
	it("buildMenuItems disables push and create PR when no origin remote exists", () => {
		const items = buildMenuItems(status({ hasUpstream: false, pr: null, aheadCount: 2 }), false, false);
		assert.deepEqual(items, [
			{
				id: "commit",
				label: "Commit",
...
describe("buildGitActionProgressStages", () => {
	...
	it("shows only PR progress when create-pr can skip the push", () => {
		...
		assert.deepEqual(stages, ["Preparing PR...", "Generating PR content...", "Creating GitHub pull request..."]);
	});
	...
	it("includes commit stages for commit+push when working tree is dirty", () => {
		...
		assert.deepEqual(stages, ["Generating commit message...", "Committing...", "Pushing to origin/feature/test..."]);
	});
