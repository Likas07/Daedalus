...
import { stackedThreadToast, type ThreadToastData, toastManager } from "~/components/ui/toast";
...
export default function GitActionsControl({ gitCwd, activeThreadRef, draftId }: GitActionsControlProps) {
	const activeEnvironmentId = activeThreadRef?.environmentId ?? null;
	const threadToastData = useMemo(
		() => (activeThreadRef ? { threadRef: activeThreadRef } : undefined),
		[activeThreadRef],
	);
	const activeServerThreadSelector = useMemo(() => createThreadSelectorByRef(activeThreadRef), [activeThreadRef]);
	const activeServerThread = useStore(activeServerThreadSelector);
	const activeDraftThread = useComposerDraftStore((store) =>
		draftId ? store.getDraftSession(draftId) : activeThreadRef ? store.getDraftThreadByRef(activeThreadRef) : null,
	);
	...
	const [pendingDefaultBranchAction, setPendingDefaultBranchAction] = useState<PendingDefaultBranchAction | null>(
		null,
	);
	...
	const persistThreadBranchSync = useCallback(
		...
		[activeDraftThread, activeServerThread, activeThreadRef, draftId, setDraftThreadContext, setThreadBranch],
	);
	...
	const isPullRunning = useIsMutating({ mutationKey: gitMutationKeys.pull(activeEnvironmentId, gitCwd) }) > 0;
	const isGitActionRunning = isRunStackedActionRunning || isPullRunning;
	const isSelectingWorktreeBase =
		!activeServerThread && activeDraftThread?.envMode === "worktree" && activeDraftThread.worktreePath === null;
	...
	useEffect(() => {
		...
		const scheduleRefreshCurrentGitStatus = () => {
			...
			refreshTimeout = window.setTimeout(() => {
				refreshTimeout = null;
				void refreshGitStatus({ environmentId: activeEnvironmentId, cwd: gitCwd }).catch(() => undefined);
	...
	runGitActionWithToast = useEffectEvent(
		async ({
			...
			const includesCommit =
				actionCanCommit && (action === "commit" || !!actionStatus?.hasWorkingTreeChanges || featureBranch);
			if (
	...
	const runQuickAction = () => {
		...
		if (quickAction.kind === "run_pull") {
			...
			void toastManager.promise<Awaited<ReturnType<typeof pullMutation.mutateAsync>>, ThreadToastData>(promise, {
	...
	return (
		<>
			{!isRepo ? (
				<Button variant="outline" size="xs" disabled={initMutation.isPending} onClick={() => initMutation.mutate()}>
			...
			>
				<DialogPopup>
					...
					<DialogFooter>
						...
						<Button variant="outline" size="sm" disabled={noneSelected} onClick={runDialogActionOnNewBranch}>
			...
			>
				<DialogPopup className="max-w-xl">
					<DialogHeader>
						<DialogTitle>{pendingDefaultBranchActionCopy?.title ?? "Run action on default branch?"}</DialogTitle>
