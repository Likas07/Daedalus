...
describe("composer logic", () => {
	test("detects path and slash triggers", () => {
		expect(detectComposerTrigger("read @src/app", 13)).toEqual({
			kind: "path",
			query: "src/app",
			rangeStart: 5,
			rangeEnd: 13,
	...
	test("maps keyboard parity for Enter-to-send and Shift+Enter newline", () => {
		expect(shouldSubmitComposerKey({ key: "Enter", shiftKey: false, isComposing: false } as KeyboardEvent)).toBe(
			true,
		);
		expect(shouldSubmitComposerKey({ key: "Enter", shiftKey: true, isComposing: false } as KeyboardEvent)).toBe(
			false,
		);
		expect(shouldSubmitComposerKey({ key: "Enter", shiftKey: false, isComposing: true } as KeyboardEvent)).toBe(
			false,
		);
	...
	test("normalizes context", () => {
		...
		expect(
			fileMentionPaths([
				{ path: "a", label: "a", kind: "file" },
				{ path: "a", label: "a", kind: "file" },
			]),
		).toEqual(["a"]);
	});
	test("validates attachments", () => {
		expect(validateAttachmentFile({ name: "x.bin", size: 1, type: "application/octet-stream" } as File)).toContain(
			"not a supported",
		);
