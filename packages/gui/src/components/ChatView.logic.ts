...
import type { ComposerImageAttachment, DraftThreadEnvMode, DraftThreadState } from "../composerDraftStore";
import {
...
export function reconcileMountedTerminalThreadIds(input: {
	...
	const nextThreadIds =
		hiddenThreadIds.length > maxHiddenThreadCount ? hiddenThreadIds.slice(-maxHiddenThreadCount) : hiddenThreadIds;

	if (input.activeThreadId && input.activeThreadTerminalOpen && !nextThreadIds.includes(input.activeThreadId)) {
...
export function cloneComposerImageForRetry(image: ComposerImageAttachment): ComposerImageAttachment {
...
export async function waitForStartedServerThread(threadRef: ScopedThreadRef, timeoutMs = 1_000): Promise<boolean> {
