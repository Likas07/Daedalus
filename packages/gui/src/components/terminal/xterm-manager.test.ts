...
import { getManagedXterm, resetXtermManagerForTests, type FitAddonLike, type TerminalLike, type XtermAddonLike } from "./xterm-manager";
...
class FakeTerminal implements TerminalLike {
	...
	open(): void { this.opened++; }
	write(data: string): void { this.writes += data; }
	clear(): void { this.writes = ""; }
	dispose(): void { this.disposed = true; }
	onData(handler: (data: string) => void): { dispose(): void } { this.#handler = handler; return { dispose() {} }; }
	loadAddon(addon: XtermAddonLike): void { this.addons.push(addon); }
	emit(data: string): void { this.#handler(data); }
}
class FakeFitAddon implements FitAddonLike { fits = 0; fit(): void { this.fits++; } }
...
describe("xterm manager", () => {
	test("keeps instances, replays delta history, and forwards input", async () => {
		const input: string[] = [];
		const runtime = { sendTerminalInput: async (_id: string, data: string) => input.push(data), resizeTerminal: async () => {} };
		const managed = getManagedXterm("term-1", runtime, { TerminalCtor: FakeTerminal, FitAddonCtor: FakeFitAddon });
		expect(getManagedXterm("term-1", runtime, { TerminalCtor: FakeTerminal, FitAddonCtor: FakeFitAddon })).toBe(managed);
	...
	test("fit reports current terminal dimensions", async () => {
		...
		const runtime = { sendTerminalInput: async () => {}, resizeTerminal: async (_id: string, size: { cols: number; rows: number }) => sizes.push(size) };
	...
	test("loads fit, search, serialize, and web links addons", () => {
		...
		const managed = getManagedXterm("term-3", runtime, { TerminalCtor: FakeTerminal, FitAddonCtor: FakeFitAddon, SearchAddonCtor: FakeAddon, SerializeAddonCtor: FakeAddon, WebLinksAddonCtor: FakeAddon });
