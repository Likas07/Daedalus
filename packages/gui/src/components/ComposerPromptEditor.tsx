...
import { selectionTouchesMentionBoundary, splitPromptIntoComposerSegments } from "~/composer-editor-mentions";
import {
...
function ComposerMentionDecorator(props: { path: string }) {
	...
	return (
		<Tooltip>
			...
			<TooltipPopup side="top" className="max-w-[30rem] whitespace-normal leading-tight wrap-anywhere">
				{props.path}
			</TooltipPopup>
		</Tooltip>
	);
...
function resolveSkillDescription(skill: Pick<ServerProviderSkill, "shortDescription" | "description">): string | null {
...
function skillMetadataByName(skills: ReadonlyArray<ServerProviderSkill>): ReadonlyMap<string, ComposerSkillMetadata> {
	return new Map(
		skills.map((skill) => [
			skill.name,
			{
...
class ComposerSkillNode extends DecoratorNode<ReactElement> {
	...
	static override clone(node: ComposerSkillNode): ComposerSkillNode {
		return new ComposerSkillNode(node.__skillName, node.__skillLabel, node.__skillDescription, node.__key);
	}
	...
	constructor(skillName: string, skillLabel: string, skillDescription: string | null, key?: NodeKey) {
		super(key);
	...
	override decorate(): ReactElement {
		return <ComposerSkillDecorator skillLabel={this.__skillLabel} skillDescription={this.__skillDescription} />;
	}
}
...
function $createComposerTerminalContextNode(context: TerminalContextDraft): ComposerTerminalContextNode {
	return $applyNodeReplacement(new ComposerTerminalContextNode(context));
}

type ComposerInlineTokenNode = ComposerMentionNode | ComposerSkillNode | ComposerTerminalContextNode;
...
function getComposerNodeExpandedTextLength(node: LexicalNode): number {
	...
	if ($isElementNode(node)) {
		return node.getChildren().reduce((total, child) => total + getComposerNodeExpandedTextLength(child), 0);
	}
	return 0;
}
...
function ComposerCommandKeyPlugin(props: {
	...
	useEffect(() => {
		const handleCommand = (key: "ArrowDown" | "ArrowUp" | "Enter" | "Tab", event: KeyboardEvent | null): boolean => {
	...
	const applySurroundInsertion = useCallback(
		(inputData: string): boolean => {
			...
			editor.update(() => {
				...
				const selectionStart = collapseExpandedComposerCursor(nextValue, selectionSnapshot.expandedStart);
			...
			queueMicrotask(() => {
				editor.update(
					() => {
						...
						$setSelectionRangeAtComposerOffsets(replacementStart + 1, replacementStart + 1 + selectedText.length);
						pendingSurroundSelectionRef.current = null;
						pendingDeadKeySelectionRef.current = null;
					},
...
function ComposerPromptEditorInner({
	...
	const terminalContextActions = useMemo(() => ({ onRemoveTerminalContext }), [onRemoveTerminalContext]);
	...
	const readSnapshot = useCallback((): {
		...
		editor.getEditorState().read(() => {
			...
			const fallbackExpandedCursor = clampExpandedCursor(nextValue, snapshotRef.current.expandedCursor);
	...
	const handleEditorChange = useCallback((editorState: EditorState) => {
		editorState.read(() => {
			...
			onChangeRef.current(nextValue, nextCursor, nextExpandedCursor, cursorAdjacentToMention, terminalContextIds);
		});
	}, []);
