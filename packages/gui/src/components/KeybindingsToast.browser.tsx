...
function queryToastTitles(): string[] {
	return Array.from(document.querySelectorAll('[data-slot="toast-title"]')).map((el) => el.textContent ?? "");
}
...
async function waitForInitialWsSubscriptions(): Promise<void> {
	await vi.waitFor(
		() => {
			expect(rpcHarness.requests.some((request) => request._tag === WS_METHODS.subscribeServerLifecycle)).toBe(true);
			expect(rpcHarness.requests.some((request) => request._tag === WS_METHODS.subscribeServerConfig)).toBe(true);
		},
		{ timeout: 8_000, interval: 16 },
	);
}
...
async function mountApp(): Promise<{ cleanup: () => Promise<void> }> {
	...
	const router = getRouter(createMemoryHistory({ initialEntries: [`/${LOCAL_ENVIRONMENT_ID}/${THREAD_ID}`] }));
...
describe("Keybindings update toast", () => {
	...
	beforeEach(async () => {
		await rpcHarness.reset({
			...
			getInitialStreamValues: (request) => {
				...
				if (request._tag === ORCHESTRATION_WS_METHODS.subscribeThread && request.threadId === THREAD_ID) {
					return [
						{
							kind: "snapshot",
							snapshot: {
	...
	it("shows a warning toast when keybinding config has issues", async () => {
		...
		try {
			sendServerConfigUpdatedPush([{ kind: "keybindings.malformed-config", message: "Expected JSON array" }]);
