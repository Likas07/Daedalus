...
export function resolveThreadPr(threadBranch: string | null, gitStatus: GitStatusResult | null): ThreadPr | null {
...
export function terminalStatusFromRunningIds(runningTerminalIds: string[]): TerminalStatusIndicator | null {
...
export function ThreadStatusLabel({ status, compact = false }: { status: ThreadStatusPill; compact?: boolean }) {
	if (compact) {
		return (
			<span
				title={status.label}
			...
			>
				<span className={`size-[9px] rounded-full ${status.dotClass} ${status.pulse ? "animate-pulse" : ""}`} />
	...
	return (
		<span title={status.label} className={`inline-flex items-center gap-1 text-[10px] ${status.colorClass}`}>
...
export function ThreadRowLeadingStatus({ thread }: { thread: SidebarThreadSummary }) {
	...
	const lastVisitedAt = useUiStateStore((state) => state.threadLastVisitedAtById[scopedThreadKey(threadRef)]);
