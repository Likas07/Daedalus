...
import { __resetEnvironmentApiOverridesForTests, __setEnvironmentApiOverrideForTests } from "../environmentApi";
import {
...
function createMockEnvironmentApi(input: {
	...
	return {
		...
		orchestration: {
			...
			subscribeThread: (() => () => undefined) as EnvironmentApi["orchestration"]["subscribeThread"],
		},
	};
}
...
function addThreadToSnapshot(snapshot: OrchestrationReadModel, threadId: ThreadId): OrchestrationReadModel {
	return {
		...snapshot,
...
function toShellThread(thread: OrchestrationReadModel["threads"][number]) {
	return {
		...
		latestUserMessageAt: thread.messages.findLast((message) => message.role === "user")?.createdAt ?? null,
...
function sendShellThreadUpsert(
	...
	const shellThread =
		options?.session !== undefined ? toShellThread({ ...thread, session: options.session }) : toShellThread(thread);
...
async function waitForWsClient(): Promise<void> {
	await vi.waitFor(
		() => {
			expect(wsRequests.some((request) => request._tag === ORCHESTRATION_WS_METHODS.subscribeShell)).toBe(true);
			expect(wsRequests.some((request) => request._tag === WS_METHODS.subscribeServerLifecycle)).toBe(true);
			expect(wsRequests.some((request) => request._tag === WS_METHODS.subscribeServerConfig)).toBe(true);
		},
...
async function promoteDraftThreadViaDomainEvent(threadId: ThreadId): Promise<void> {
	...
	await vi.waitFor(
		() => {
			expect(useComposerDraftStore.getState().draftThreadsByThreadKey[threadKeyFor(threadId)]).toBe(undefined);
		},
		{ timeout: 8_000, interval: 16 },
	);
}
...
function createSnapshotWithPlanFollowUpPrompt(options?: {
	...
	const planMarkdown = options?.planMarkdown ?? "# Follow-up plan\n\n- Keep the composer footer stable on resize.";
...
function resolveWsRpc(body: NormalizedWsRpcRequestBody): unknown {
	...
	if (tag === WS_METHODS.terminalOpen) {
		return {
			...
			worktreePath:
				typeof body.worktreePath === "string" ? body.worktreePath : body.worktreePath === null ? null : null,
			status: "running",
			pid: 123,
			history: "",
			exitCode: null,
...
async function waitForElement<T extends Element>(query: () => T | null, errorMessage: string): Promise<T> {
...
async function pressComposerKey(key: string): Promise<void> {
	...
	if (typeof document.execCommand === "function" && document.execCommand("insertText", false, key)) {
		await waitForLayout();
		return;
	}
...
async function waitForComposerText(expectedText: string): Promise<void> {
	await vi.waitFor(
		() => {
			expect(useComposerDraftStore.getState().draftsByThreadKey[THREAD_KEY]?.prompt ?? "").toBe(expectedText);
		},
		{ timeout: 8_000, interval: 16 },
	);
}
...
async function waitForButtonContainingText(text: string): Promise<HTMLButtonElement> {
	return waitForElement(() => findButtonContainingText(text), `Unable to find button containing "${text}".`);
}
...
async function waitForInteractionModeButton(expectedLabel: "Build" | "Plan"): Promise<HTMLButtonElement> {
	return waitForElement(
		() =>
...
async function waitForNewThreadShortcutLabel(): Promise<void> {
	...
	const shortcutLabel = isMacPlatform(navigator.platform) ? "New thread (⇧⌘O)" : "New thread (Ctrl+Shift+O)";
...
describe("ChatView timeline estimator parity (full app)", () => {
	...
	it("does not leak a server worktree path into drawer runtime env when launch context clears it", async () => {
		...
		try {
			await vi.waitFor(
				() => {
					const openRequest = wsRequests.find((request) => request._tag === WS_METHODS.terminalOpen) as
						| {
								_tag: string;
								cwd?: string;
	...
	it("runs project scripts from local draft threads at the project cwd", async () => {
		...
		try {
			...
			await vi.waitFor(
				() => {
					const openRequest = wsRequests.find((request) => request._tag === WS_METHODS.terminalOpen);
					expect(openRequest).toMatchObject({
						_tag: WS_METHODS.terminalOpen,
						threadId: THREAD_ID,
						cwd: "/repo/project",
			...
			await vi.waitFor(
				() => {
					const writeRequest = wsRequests.find((request) => request._tag === WS_METHODS.terminalWrite);
	...
	it("lets the server own setup after preparing a pull request worktree thread", async () => {
		...
		try {
			...
			expect(
				wsRequests.some((request) => request._tag === WS_METHODS.terminalWrite && request.data === "bun install\r"),
			).toBe(false);
		} finally {
			await mounted.cleanup();
		}
	...
	it("sends bootstrap turn-starts and waits for server setup on first-send worktree drafts", async () => {
		...
		try {
			...
			expect(wsRequests.some((request) => request._tag === WS_METHODS.gitCreateWorktree)).toBe(false);
			expect(
				wsRequests.some(
					(request) =>
			...
			await vi.waitFor(
				() => {
					const turnStartRequest = wsRequests.find(
						(request) =>
							request._tag === ORCHESTRATION_WS_METHODS.dispatchCommand && request.type === "thread.turn.start",
					) as
						| {
								_tag: string;
								type?: string;
	...
	it("shows the send state once bootstrap dispatch is in flight", async () => {
		...
		try {
			...
			await vi.waitFor(
				() => {
					expect(wsRequests.some((request) => request._tag === ORCHESTRATION_WS_METHODS.dispatchCommand)).toBe(
						true,
					);
	...
	it("toggles plan mode with Shift+Tab only while the composer is focused", async () => {
		...
		try {
			...
			await vi.waitFor(
				async () => {
					expect((await waitForInteractionModeButton("Plan")).title).toContain("return to normal build mode");
				},
				{ timeout: 8_000, interval: 16 },
			);
	...
	it("uses the active draft route session when changing the base branch", async () => {
		...
		try {
			...
			await vi.waitFor(
				() => {
					expect(useComposerDraftStore.getState().getDraftSession(activeDraftId)?.branch).toBe("release/next");
					expect(useComposerDraftStore.getState().getDraftSession(staleDraftId)?.branch).toBe("main");
				},
				{ timeout: 8_000, interval: 16 },
			);
		...
		try {
			...
			await vi.waitFor(
				() => {
					...
					expect(popupSpans.some((element) => element.textContent?.trim() === "feature/current")).toBe(true);
	...
	it("warns when sending text while omitting expired terminal pills", async () => {
		...
		useComposerDraftStore.getState().setPrompt(THREAD_REF, `yoo${INLINE_TERMINAL_CONTEXT_PLACEHOLDER}waddup`);
	...
	it("hides the archive action when the pointer leaves a thread row", async () => {
		...
		try {
			...
			expect(archiveAction, "Archive button should render inside a visibility wrapper.").not.toBeNull();
	...
	it("canonicalizes promoted draft threads to the server thread route", async () => {
		...
		try {
			...
			await vi.waitFor(
				() => {
					expect(useComposerDraftStore.getState().draftThreadsByThreadKey[newDraftId]).toBe(undefined);
				},
				{ timeout: 8_000, interval: 16 },
			);
	...
	it("renders the configurable shortcut and runs a command from the sidebar trigger", async () => {
		...
		try {
			...
			await expect.element(palette.getByText("New thread in Project", { exact: true })).toBeInTheDocument();
	...
	it("filters command palette results as the user types", async () => {
		...
		try {
			...
			await expect.element(palette.getByText("New thread in Project", { exact: true })).not.toBeInTheDocument();
		} finally {
			await mounted.cleanup();
		}
	});
	...
	it("adds a project from browse mode with Enter when no directory is highlighted", async () => {
		...
		try {
			...
			await expect.element(palette.getByRole("button", { name: "Add (Enter)" })).toBeInTheDocument();
			...
			await vi.waitFor(
				() => {
					const dispatchRequest = wsRequests.find(
						(request) =>
							request._tag === ORCHESTRATION_WS_METHODS.dispatchCommand && request.type === "project.create",
					) as
						| {
								_tag: string;
								type?: string;
		...
		try {
			...
			await vi.waitFor(
				() => {
					expect(
						wsRequests.some(
							(request) => request._tag === WS_METHODS.filesystemBrowse && request.partialPath === "~/",
						),
					).toBe(true);
				},
				{ timeout: 8_000, interval: 16 },
			...
			await vi.waitFor(
				() => {
					expect(
						wsRequests.some(
							(request) =>
								request._tag === WS_METHODS.filesystemBrowse && request.partialPath === "~/Development/",
						),
					).toBe(true);
				},
				{ timeout: 8_000, interval: 16 },
	...
	it("shows create-folder affordances for missing project paths", async () => {
		...
		try {
			...
			await expect.element(palette.getByRole("button", { name: "Create & Add (Enter)" })).toBeInTheDocument();
	...
	it("does not show create affordances for an existing directory with a trailing slash", async () => {
		...
		try {
			...
			await vi.waitFor(
				() => {
					expect(
						wsRequests.some(
							(request) =>
								request._tag === WS_METHODS.filesystemBrowse && request.partialPath === "~/Development/codex/",
						),
					).toBe(true);
				},
				{ timeout: 8_000, interval: 16 },
			...
			await expect.element(palette.getByRole("button", { name: "Add (Enter)" })).toBeInTheDocument();
			await expect.element(palette.getByRole("button", { name: "Create & Add (Enter)" })).not.toBeInTheDocument();
	...
	it("selects an environment before browsing when multiple environments are available", async () => {
		...
		try {
			...
			await expect.element(palette.getByText("This device", { exact: true }).first()).toBeInTheDocument();
	...
	it("adds a project from browse mode with Mod+Enter when a directory is highlighted", async () => {
		...
		try {
			...
			const addButtonLabel = isMacPlatform(navigator.platform) ? "Add (\u2318 Enter)" : "Add (Ctrl Enter)";
			await vi.waitFor(
				() => {
			...
			await expect.element(palette.getByRole("button", { name: addButtonLabel })).toBeInTheDocument();
	...
	it("keeps project-context thread matches available when searching by project name", async () => {
		...
		try {
			...
			await expect.element(palette.getByText("Release checklist", { exact: true })).toBeInTheDocument();
		} finally {
			await mounted.cleanup();
		}
	});
	...
	it("searches projects by path and opens the latest thread for that project", async () => {
		...
		try {
			...
			await expect.element(palette.getByText("Docs Portal", { exact: true })).toBeInTheDocument();
			await expect.element(palette.getByText("/repo/clients/docs-portal", { exact: true })).toBeInTheDocument();
			...
			expect(
				useComposerDraftStore.getState().getDraftThread(threadRefFor("thread-secondary-project" as ThreadId)),
			).toBeNull();
		} finally {
			await mounted.cleanup();
		}
	...
	it("filters archived threads out of command palette search results", async () => {
		...
		try {
			...
			await expect.element(palette.getByText("Archived Docs Notes", { exact: true })).not.toBeInTheDocument();
		} finally {
			await mounted.cleanup();
		}
	});
	...
	it("uses the active worktree path when saving a proposed plan to the workspace", async () => {
		...
		try {
			...
			await vi.waitFor(
				() => {
					expect(document.body.textContent).toContain("Enter a path relative to /repo/worktrees/plan-thread.");
				},
				{ timeout: 8_000, interval: 16 },
			);
		} finally {
	...
	it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
		const mounted = await mountChatView({
			...
			snapshot: createSnapshotWithPlanFollowUpPrompt({
				...
				planMarkdown:
					"# Imaginary Long-Range Plan: Daedalus Adaptive Orchestration and Safe-Delay Execution Initiative",
			}),
		});
	...
	it("opens the model picker when selecting /model", async () => {
		...
		try {
			...
			await vi.waitFor(() => {
				const searchInput = document.querySelector<HTMLInputElement>('input[placeholder="Search models..."]');
	...
	it("toggles the model picker and shows jump keys immediately from the shortcut", async () => {
		...
		try {
			...
			await vi.waitFor(() => {
				expect(
					Array.from(document.querySelectorAll<HTMLElement>('.model-picker-list [data-slot="kbd"]')).some(
						(element) => element.textContent?.trim() === jumpLabel,
					),
				).toBe(true);
			});
