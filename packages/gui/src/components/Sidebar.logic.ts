...
import { getThreadSortTimestamp, sortThreads, type ThreadSortInput, toSortableTimestamp } from "../lib/threadSort";
...
export function isContextMenuPointerDown(input: { button: number; ctrlKey: boolean; isMac: boolean }): boolean {
	if (input.button === 2) return true;
	return input.isMac && input.button === 0 && input.ctrlKey;
}

export function resolveThreadRowClassName(input: { isActive: boolean; isSelected: boolean }): string {
	const baseClassName =
...
>(projects: readonly TProject[], threads: readonly TThread[], sortOrder: SidebarProjectSortOrder): TProject[] {
	if (sortOrder === "manual") {
		return [...projects];
	}
	...
	return [...projects].toSorted((left, right) => {
		const rightTimestamp = getProjectSortTimestamp(right, threadsByProjectId.get(right.id) ?? [], sortOrder);
		const leftTimestamp = getProjectSortTimestamp(left, threadsByProjectId.get(left.id) ?? [], sortOrder);
		const byTimestamp = rightTimestamp === leftTimestamp ? 0 : rightTimestamp > leftTimestamp ? 1 : -1;
