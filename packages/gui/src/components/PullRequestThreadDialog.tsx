...
import { gitPreparePullRequestThreadMutationOptions, gitResolvePullRequestQueryOptions } from "~/lib/gitReactQuery";
...
export function PullRequestThreadDialog({
	...
	const [debouncedReference, referenceDebouncer] = useDebouncedValue(reference, { wait: 450 }, (debouncerState) => ({
		isPending: debouncerState.isPending,
	}));
	...
	const handleConfirm = useCallback(
		...
		[cwd, onOpenChange, onPrepared, parsedReference, preparePullRequestThreadMutation, resolvedPullRequest, threadId],
	);
		...
		>
			<DialogPopup className="max-w-xl">
				<DialogHeader>
					...
					<DialogDescription>
						Resolve a GitHub pull request, then create the draft thread in the main repo or in a dedicated
						worktree.
					</DialogDescription>
				</DialogHeader>
				...
				<DialogFooter>
					...
					<Button
						...
						disabled={!cwd || !resolvedPullRequest || isResolving || preparePullRequestThreadMutation.isPending}
					>
