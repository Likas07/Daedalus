...
function makeTerminalEvent(type: TerminalEvent["type"], overrides: Partial<TerminalEvent> = {}): TerminalEvent {
	const base = {
		threadId: THREAD_ID,
		terminalId: "default",
...
describe("terminalStateStore actions", () => {
	...
	it("opens and splits terminals into the active group", () => {
		...
		expect(terminalState.terminalGroups).toEqual([{ id: "group-default", terminalIds: ["default", "terminal-2"] }]);
	});
	...
	it("caps splits at four terminals per group", () => {
		...
		expect(terminalState.terminalIds).toEqual(["default", "terminal-2", "terminal-3", "terminal-4"]);
	...
	it("keeps state isolated per environment when raw thread ids collide", () => {
		...
		expect(
			selectThreadTerminalState(useTerminalStateStore.getState().terminalStateByThreadKey, THREAD_REF).terminalOpen,
		).toBe(true);
		expect(
			selectThreadTerminalState(useTerminalStateStore.getState().terminalStateByThreadKey, OTHER_THREAD_REF)
				.terminalIds,
	...
	it("tracks and clears terminal subprocess activity", () => {
		...
		expect(
			selectThreadTerminalState(useTerminalStateStore.getState().terminalStateByThreadKey, THREAD_REF)
				.runningTerminalIds,
		).toEqual(["terminal-2"]);
	...
	it("resets to default and clears persisted entry when closing the last terminal", () => {
		...
		expect(useTerminalStateStore.getState().terminalStateByThreadKey[scopedThreadKey(THREAD_REF)]).toBeUndefined();
		expect(
			selectThreadTerminalState(useTerminalStateStore.getState().terminalStateByThreadKey, THREAD_REF).terminalIds,
		).toEqual(["default"]);
	});
	...
	it("applies started terminal events to terminal state, launch context, and event buffer", () => {
		...
		expect(useTerminalStateStore.getState().terminalLaunchContextByThreadKey[scopedThreadKey(THREAD_REF)]).toEqual({
			cwd: "/tmp/worktree",
			worktreePath: "/tmp/worktree",
		});
