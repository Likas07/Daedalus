import { isExplicitRelativePath, isUncPath, isWindowsAbsolutePath, isWindowsDrivePath } from "@t3tools/shared/path";
...
function trimTrailingPathSeparators(value: string): string {
	...
	const trimmed = getAbsolutePathKind(value) === "unix" ? value.replace(/\/+$/g, "") : value.replace(/[\\/]+$/g, "");
	if (trimmed.length === 0) {
		return value;
	}
...
export function findProjectByPath<T extends { cwd: string }>(
	...
	return projects.find((project) => normalizeProjectPathForComparison(project.cwd) === normalizedCandidate);
}
