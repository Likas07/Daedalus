import { type EnvironmentApi, EnvironmentId, ThreadId } from "@t3tools/contracts";
...
function mockNativeApi(input: { getTurnDiff: ReturnType<typeof vi.fn>; getFullThreadDiff: ReturnType<typeof vi.fn> }) {
...
describe("checkpointDiffQueryOptions", () => {
	...
	it("fails fast on invalid range and does not call provider RPC", async () => {
		...
		await expect(queryClient.fetchQuery(options)).rejects.toThrow("Checkpoint diff is unavailable.");
		expect(getTurnDiff).not.toHaveBeenCalled();
		expect(getFullThreadDiff).not.toHaveBeenCalled();
	});
	...
	it("retries checkpoint-not-ready errors longer than generic failures", () => {
		...
		expect(retry(11, new Error("Filesystem checkpoint is unavailable for turn 2 in thread thread-1."))).toBe(true);
		expect(retry(12, new Error("Filesystem checkpoint is unavailable for turn 2 in thread thread-1."))).toBe(false);
	...
	it("backs off longer for checkpoint-not-ready errors", () => {
		...
		const checkpointDelay = retryDelay(4, new Error("Checkpoint turn count 2 exceeds current turn count 1."));
