...
function normalizeCheckpointErrorMessage(error: unknown): string {
	...
	if (lower.includes("checkpoint unavailable for thread") || lower.includes("checkpoint invariant violation")) {
...
export function checkpointDiffQueryOptions(input: CheckpointDiffQueryInput) {
	...
	return queryOptions({
		...
		enabled: (input.enabled ?? true) && !!input.environmentId && !!input.threadId && decodedRequest._tag === "Some",
		staleTime: Infinity,
