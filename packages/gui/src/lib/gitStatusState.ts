...
const gitStatusStateAtom = Atom.family((key: string) => {
	...
	return Atom.make(INITIAL_GIT_STATUS_STATE).pipe(Atom.keepAlive, Atom.withLabel(`git-status:${key}`));
});
...
function readResolvedGitStatusClient(target: GitStatusTarget): ResolvedGitStatusClient | null {
	...
	return connection ? { clientIdentity: connection.environmentId, client: connection.client.git } : null;
}
...
export function refreshGitStatus(target: GitStatusTarget, client?: GitStatusClient): Promise<GitStatusResult | null> {
	const targetKey = getGitStatusTargetKey(target);
...
export function useGitStatus(target: GitStatusTarget): GitStatusState {
	...
	const state = useAtomValue(targetKey !== null ? gitStatusStateAtom(targetKey) : EMPTY_GIT_STATUS_ATOM);
...
function subscribeToGitStatusTarget(
	...
	const unsubscribeRegistry = providedClient ? NOOP : subscribeEnvironmentConnections(syncClientSubscription);
