...
describe("terminalContext", () => {
	...
	it("replaces inline placeholders with inline terminal labels before appending context blocks", () => {
		expect(
			appendTerminalContextsToPrompt(`Investigate ${INLINE_TERMINAL_CONTEXT_PLACEHOLDER} carefully`, [
				makeContext(),
			]),
		).toBe(
			[
	...
	it("formats and materializes inline terminal labels from placeholder positions", () => {
		...
		expect(
			materializeInlineTerminalContextPrompt(`Investigate ${INLINE_TERMINAL_CONTEXT_PLACEHOLDER} carefully`, [
				makeContext(),
			]),
