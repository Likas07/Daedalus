import type { EnvironmentId, GitActionProgressEvent, GitStackedAction, ThreadId } from "@t3tools/contracts";
import { infiniteQueryOptions, mutationOptions, type QueryClient, queryOptions } from "@tanstack/react-query";
...
function invalidateGitBranchQueries(queryClient: QueryClient, environmentId: EnvironmentId | null, cwd: string | null) {
	if (cwd === null) {
...
export function gitResolvePullRequestQueryOptions(input: {
	...
	return queryOptions({
		queryKey: ["git", "pull-request", input.environmentId ?? null, input.cwd, input.reference] as const,
		queryFn: async () => {
...
export function gitCreateWorktreeMutationOptions(input: {
	...
	return mutationOptions({
		...
		mutationFn: (args: Parameters<ReturnType<typeof ensureEnvironmentApi>["git"]["createWorktree"]>[0]) => {
...
export function gitRemoveWorktreeMutationOptions(input: {
	...
	return mutationOptions({
		...
		mutationFn: (args: Parameters<ReturnType<typeof ensureEnvironmentApi>["git"]["removeWorktree"]>[0]) => {
			if (!input.environmentId) {
...
export function gitPreparePullRequestThreadMutationOptions(input: {
	...
	return mutationOptions({
		...
		mutationFn: async (args: { reference: string; mode: "local" | "worktree"; threadId?: ThreadId }) => {
