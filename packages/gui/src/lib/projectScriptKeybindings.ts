...
function normalizeProjectScriptKeybindingInput(keybinding: string | null | undefined): string | null {
...
export function keybindingValueForCommand(
	...
	for (let index = keybindings.length - 1; index >= 0; index -= 1) {
		...
		const keyToken =
			binding.shortcut.key === " " ? "space" : binding.shortcut.key === "escape" ? "esc" : binding.shortcut.key;
		parts.push(keyToken);
		return parts.join("+");
	}
	return null;
