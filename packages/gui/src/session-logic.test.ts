import { EventId, MessageId, type OrchestrationThreadActivity, ThreadId, TurnId } from "@t3tools/contracts";
...
describe("deriveWorkLogEntries", () => {
	...
	it("unwraps PowerShell command wrappers for displayed command text", () => {
		...
		const [entry] = deriveWorkLogEntries(activities, undefined);
		expect(entry?.command).toBe("bun run lint");
		expect(entry?.rawCommand).toBe("\"C:\\Program Files\\PowerShell\\7\\pwsh.exe\" -Command 'bun run lint'");
	});
	...
	it("unwraps PowerShell command wrappers from argv-style command payloads", () => {
		...
		expect(entry?.rawCommand).toBe('"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "rg -n foo ."');
	});
	...
	it("extracts changed file paths for file-change tool activities", () => {
		...
		const [entry] = deriveWorkLogEntries(activities, undefined);
		expect(entry?.changedFiles).toEqual(["apps/web/src/components/ChatView.tsx", "apps/web/src/session-logic.ts"]);
	});
