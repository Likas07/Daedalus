...
function modShortcut(key: string, overrides: Partial<Omit<KeybindingShortcut, "key">> = {}): KeybindingShortcut {
	return {
		key,
		metaKey: false,
		ctrlKey: false,
...
describe("isTerminalToggleShortcut", () => {
	...
	it("matches Ctrl+J on non-macOS", () => {
		assert.isTrue(isTerminalToggleShortcut(event({ ctrlKey: true }), DEFAULT_BINDINGS, { platform: "Win32" }));
	});
...
describe("shortcutLabelForCommand", () => {
	...
	it("returns effective labels for non-terminal commands", () => {
		...
		assert.strictEqual(shortcutLabelForCommand(DEFAULT_BINDINGS, "commandPalette.toggle", "MacIntel"), "⌘K");
		assert.strictEqual(shortcutLabelForCommand(DEFAULT_BINDINGS, "modelPicker.toggle", "Linux"), "Ctrl+Shift+M");
		assert.strictEqual(shortcutLabelForCommand(DEFAULT_BINDINGS, "editor.openFavorite", "Linux"), "Ctrl+O");
		assert.strictEqual(shortcutLabelForCommand(DEFAULT_BINDINGS, "thread.jump.3", "MacIntel"), "⌘3");
		assert.strictEqual(shortcutLabelForCommand(DEFAULT_BINDINGS, "thread.previous", "Linux"), "Ctrl+Shift+[");
		assert.strictEqual(
...
describe("terminalDeleteShortcutData", () => {
	it("maps Cmd+Backspace on macOS to delete-to-line-start", () => {
		assert.strictEqual(terminalDeleteShortcutData(event({ key: "Backspace", metaKey: true }), "MacIntel"), "\u0015");
	});
	...
	it("ignores non-macOS platforms and modified variants", () => {
		...
		assert.isNull(terminalDeleteShortcutData(event({ key: "Backspace", metaKey: true, altKey: true }), "MacIntel"));
	});

	it("ignores non-keydown events", () => {
		assert.isNull(terminalDeleteShortcutData(event({ type: "keyup", key: "Backspace", metaKey: true }), "MacIntel"));
	});
});
...
describe("terminalNavigationShortcutData", () => {
	...
	it("rejects unsupported combinations", () => {
		assert.isNull(
			terminalNavigationShortcutData(event({ key: "ArrowLeft", shiftKey: true, altKey: true }), "MacIntel"),
		);
	...
	it("ignores non-keydown events", () => {
		assert.isNull(
			terminalNavigationShortcutData(event({ type: "keyup", key: "ArrowLeft", altKey: true }), "MacIntel"),
		);
	});
});
