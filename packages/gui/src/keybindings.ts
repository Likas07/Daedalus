...
export function shortcutLabelForCommand(
	...
	const resolvedOptions =
		typeof options === "string" ? ({ platform: options } satisfies ResolvedShortcutLabelOptions) : options;
...
export function threadTraversalDirectionFromCommand(command: string | null): "previous" | "next" | null {
...
export function modelPickerJumpCommandForIndex(index: number): ModelPickerJumpKeybindingCommand | null {
...
export function modelPickerJumpIndexFromCommand(command: string): number | null {
	const index = MODEL_PICKER_JUMP_KEYBINDING_COMMANDS.indexOf(command as ModelPickerJumpKeybindingCommand);
	return index === -1 ? null : index;
}
...
export function isTerminalClearShortcut(event: ShortcutEventLike, platform = navigator.platform): boolean {
...
export function terminalDeleteShortcutData(event: ShortcutEventLike, platform = navigator.platform): string | null {
	if (event.type !== undefined && event.type !== "keydown") {
		return null;
	}
	...
	return event.metaKey && !event.ctrlKey && !event.altKey && !event.shiftKey ? TERMINAL_DELETE_TO_LINE_START : null;
}

export function terminalNavigationShortcutData(event: ShortcutEventLike, platform = navigator.platform): string | null {
	if (event.type !== undefined && event.type !== "keydown") {
		return null;
	}
