...
export function buildPhysicalToLogicalProjectKeyMap(input: {
	...
	for (const project of input.projects) {
		mapping.set(derivePhysicalProjectKey(project), deriveLogicalProjectKeyFromSettings(project, input.settings));
	}
	return mapping;
}
...
export function buildSidebarProjectSnapshots(input: {
	...
	for (const project of input.projects) {
		...
		const remoteEnvironmentLabels = members
			.filter((member) => input.primaryEnvironmentId !== null && member.environmentId !== input.primaryEnvironmentId)
