...
import { selectProjectByRef, selectThreadByRef, selectThreadsForEnvironment, useStore } from "../store";
...
export function useThreadActions() {
	...
	const getCurrentRouteThreadRef = useCallback(() => {
		const currentRouteParams = (router.state.matches[router.state.matches.length - 1]?.params ?? {}) as Partial<
		...
		async (target: ScopedThreadRef, opts: { deletedThreadKeys?: ReadonlySet<string> } = {}) => {
			...
			const survivingThreads =
				deletedIds && deletedIds.size > 0
					...
					: threads;
			const orphanedWorktreePath = getOrphanedWorktreePathForThread(survivingThreads, threadRef.threadId);
			const displayWorktreePath = orphanedWorktreePath ? formatWorktreePathForDisplay(orphanedWorktreePath) : null;
			...
			clearProjectDraftThreadById(scopeProjectRef(threadRef.environmentId, thread.projectId), threadRef);
				...
				if (fallbackThreadId) {
					...
					if (fallbackThread) {
						await router.navigate({
							...
							params: buildThreadRouteParams(scopeThreadRef(fallbackThread.environmentId, fallbackThread.id)),
							replace: true,
						});
					} else {
