...
const showContextMenuFallbackMock =
	vi.fn<
		<T extends string>(items: readonly ContextMenuItem<T>[], position?: { x: number; y: number }) => Promise<T | null>
	>();
...
const rpcClientMock = {
	...
	terminal: {
		...
		restart: vi.fn(),
		close: vi.fn(),
		onEvent: vi.fn((listener: (event: TerminalEvent) => void) => registerListener(terminalEventListeners, listener)),
	},
	projects: {
		searchEntries: vi.fn(),
		writeFile: vi.fn(),
...
describe("wsApi", () => {
	...
	it("forwards server settings updates directly to the RPC client", async () => {
		...
		await expect(api.server.updateSettings({ enableAssistantStreaming: true })).resolves.toEqual(nextSettings);
	...
	it("forwards folder picker options to the desktop bridge", async () => {
		...
		await expect(api.dialogs.pickFolder({ initialPath: "/tmp/workspace" })).resolves.toBe("/tmp/project");
	...
	it("reads and writes persistence through the desktop bridge when available", async () => {
		...
		await api.persistence.setSavedEnvironmentSecret(EnvironmentId.make("environment-local"), "bearer-token");
	...
	it("falls back to browser storage for persistence when the desktop bridge is missing", async () => {
		...
		await expect(api.persistence.getSavedEnvironmentSecret(EnvironmentId.make("environment-local"))).resolves.toBe(
			"bearer-token",
		);
