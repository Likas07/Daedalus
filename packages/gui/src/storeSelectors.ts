...
import { type AppState, type EnvironmentState, selectEnvironmentState } from "./store";
...
export function createProjectSelectorByRef(
	...
	return (state) => (ref ? selectEnvironmentState(state, ref.environmentId).projectById[ref.projectId] : undefined);
}
...
function createScopedThreadSelector(
	...
	return (state) => {
		...
		if (previousThread && previousEnvironmentState === environmentState && previousThreadId === ref.threadId) {
...
export function createThreadSelectorAcrossEnvironments(
	...
	return createScopedThreadSelector((state) => {
		...
		for (const [environmentId, environmentState] of Object.entries(state.environmentStateById) as Array<
			[ScopedThreadRef["environmentId"], EnvironmentState]
		>) {
