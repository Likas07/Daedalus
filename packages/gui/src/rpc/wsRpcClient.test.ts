import type { GitStatusLocalResult, GitStatusRemoteResult, GitStatusStreamEvent } from "@t3tools/contracts";
...
describe("wsRpcClient", () => {
	it("reduces git status stream events into flat status snapshots", () => {
		...
		const transport = {
			...
			requestStream: vi.fn(),
			subscribe,
		} satisfies Pick<WsTransport, "dispose" | "reconnect" | "request" | "requestStream" | "subscribe">;
