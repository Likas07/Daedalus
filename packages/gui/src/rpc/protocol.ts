...
import { acknowledgeRpcRequest, clearAllTrackedRpcRequests, trackRpcRequestSent } from "./requestLatencyState";
import {
...
function composeLifecycleHandlers(handlers?: WsProtocolLifecycleHandlers): Required<WsProtocolLifecycleHandlers> {
...
export function createWsRpcProtocolLayer(url: WsRpcProtocolSocketUrlProvider, handlers?: WsProtocolLifecycleHandlers) {
	...
	const trackingWebSocketConstructorLayer = Layer.succeed(Socket.WebSocketConstructor, (socketUrl, protocols) => {
	...
	const socketLayer = Socket.layerWebSocket(resolvedUrl).pipe(Layer.provide(trackingWebSocketConstructorLayer));
