...
import { __resetClientTracingForTests, configureClientTracing } from "../observability/clientTracing";
import {
...
describe("WsTransport", () => {
	...
	it("delivers stream chunks to subscribers", async () => {
		...
		const unsubscribe = transport.subscribe((client) => client[WS_METHODS.subscribeServerLifecycle]({}), listener);
		await waitFor(() => {
			expect(sockets).toHaveLength(1);
		});
	...
	it("does not retry stream subscriptions after application-level failures", async () => {
		...
		expect(warnSpy).not.toHaveBeenCalledWith("WebSocket RPC subscription disconnected", expect.anything());
	...
	it("logs a transport disconnect once even when multiple subscriptions fail together", async () => {
		...
		const unsubscribeA = transport.subscribe(() => Stream.fail(new Error("SocketCloseError: 1006")), vi.fn(), {
			retryDelay: 10,
		});
		const unsubscribeB = transport.subscribe(() => Stream.fail(new Error("SocketCloseError: 1006")), vi.fn(), {
			retryDelay: 10,
		});
