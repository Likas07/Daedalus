...
export function listLoginShellCandidates(
	...
	const fallbackShell = platform === "darwin" ? "/bin/zsh" : platform === "linux" ? "/bin/bash" : undefined;
...
export function readPathFromLoginShell(shell: string, execFile: ExecFileSyncLike = execFileSync): string | undefined {
	return readEnvironmentFromLoginShell(shell, ["PATH"], execFile).PATH;
}

export function readPathFromLaunchctl(execFile: ExecFileSyncLike = execFileSync): string | undefined {
	try {
		return trimNonEmpty(
...
export const readEnvironmentFromLoginShell: ShellEnvironmentReader = (shell, names, execFile = execFileSync) => {
	if (names.length === 0) {
		return {};
	}
...
export function isCommandAvailable(command: string, options: CommandAvailabilityOptions = {}): boolean {
	...
	if (command.includes("/") || command.includes("\\")) {
		return commandCandidates.some((candidate) => isExecutableFile(candidate, platform, windowsPathExtensions));
	}
...
function mergeWindowsEnv(currentEnv: NodeJS.ProcessEnv, patch: Partial<Record<string, string>>): NodeJS.ProcessEnv {
...
export function resolveWindowsEnvironment(
	...
	return Object.keys(profiledPatch).length > 0 ? { ...baselinePatch, ...profiledPatch } : baselinePatch;
}
