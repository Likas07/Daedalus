 ...
 */
export function resolveAutoFeatureBranchName(existingBranchNames: readonly string[], preferredBranch?: string): string {
...
function deriveLocalBranchNameCandidatesFromRemoteRef(branchName: string, remoteName?: string): ReadonlyArray<string> {
 ...
 */
export function dedupeRemoteBranchesWithLocalMatches(branches: ReadonlyArray<GitBranch>): ReadonlyArray<GitBranch> {
	...
	return branches.filter((branch) => {
		...
		const localBranchCandidates = deriveLocalBranchNameCandidatesFromRemoteRef(branch.name, branch.remoteName);
...
export function detectGitHostingProviderFromRemoteUrl(remoteUrl: string): GitHostingProvider | null {
