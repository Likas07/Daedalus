...
function normalizeCommandValue(value: unknown): string | undefined {
	...
	const parts = value.map((entry) => asTrimmedString(entry)).filter((entry): entry is string => entry !== undefined);
...
function maybePathLike(value: string | undefined): string | undefined {
	...
	if (value.includes("/") || value.includes("\\") || value.startsWith(".") || /\.(?:[a-z0-9]{1,12})$/iu.test(value)) {
		return value;
	}
	return undefined;
}
...
function classifyToolAction(input: {
	...
	if (itemType === "file_change" || kind === "edit" || kind === "move" || kind === "delete" || kind === "write") {
		return "file_change";
	}
