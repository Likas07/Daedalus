...
describe("resolveModelSlugForProvider", () => {
	it("returns defaults when the model is missing", () => {
		...
		expect(resolveModelSlugForProvider("claudeAgent", undefined)).toBe(DEFAULT_MODEL_BY_PROVIDER.claudeAgent);
	});

	it("preserves normalized unknown models", () => {
		expect(resolveModelSlugForProvider("codex", "custom/internal-model")).toBe("custom/internal-model");
	});
});
...
describe("misc helpers", () => {
	...
	it("prefixes ultrathink prompts once", () => {
		expect(applyClaudePromptEffortPrefix("Investigate", "ultrathink")).toBe("Ultrathink:\nInvestigate");
...
describe("descriptor helpers", () => {
	...
	it("reads typed option selection values", () => {
		...
		expect(getProviderOptionBooleanSelectionValue(selection.options, "reasoningEffort")).toBeUndefined();
