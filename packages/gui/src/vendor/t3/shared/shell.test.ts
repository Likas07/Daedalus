...
describe("readPathFromLoginShell", () => {
	it("uses a shell-agnostic printenv PATH probe", () => {
		const execFile = vi.fn<
			(file: string, args: ReadonlyArray<string>, options: { encoding: "utf8"; timeout: number }) => string
...
describe("mergePathEntries", () => {
	it("prefers login-shell PATH entries and keeps inherited extras", () => {
		expect(mergePathEntries("/opt/homebrew/bin:/usr/bin", "/Users/test/.local/bin:/usr/bin", "darwin")).toBe(
...
describe("resolveWindowsEnvironment", () => {
	it("returns the baseline no-profile PATH patch when node is already available", () => {
		const readEnvironment = vi.fn((_names: ReadonlyArray<string>, options?: { loadProfile?: boolean }) =>
			options?.loadProfile ? { PATH: "C:\\Profile\\Bin" } : { PATH: "C:\\Shell\\Bin;C:\\Windows\\System32" },
		);
