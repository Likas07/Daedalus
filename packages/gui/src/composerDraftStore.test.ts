import { scopedProjectKey, scopedThreadKey, scopeProjectRef, scopeThreadRef } from "@t3tools/client-runtime";
import {
	EnvironmentId,
	type ModelSelection,
	ProjectId,
...
function threadKeyFor(threadId: ThreadId, environmentId: EnvironmentId = LEGACY_TEST_ENVIRONMENT_ID): string {
...
describe("composerDraftStore terminal contexts", () => {
	...
	it("clears terminal contexts when clearing composer content", () => {
		useComposerDraftStore.getState().addTerminalContext(threadRef, makeTerminalContext({ id: "ctx-1" }));
	...
	it("omits terminal context text from persisted drafts", () => {
		useComposerDraftStore.getState().addTerminalContext(threadRef, makeTerminalContext({ id: "ctx-persist" }));
		...
		expect(
			persistedState.draftsByThreadKey?.[threadKeyFor(threadId, TEST_ENVIRONMENT_ID)]?.terminalContexts?.[0],
		...
		expect(
			persistedState.draftsByThreadKey?.[threadKeyFor(threadId, TEST_ENVIRONMENT_ID)]?.terminalContexts?.[0]?.text,
		).toBeUndefined();
	});
...
describe("composerDraftStore project draft thread mapping", () => {
	...
	it("clears only matching project draft mapping entries", () => {
		...
		expect(useComposerDraftStore.getState().getDraftThreadByProjectRef(projectRef)?.threadId).toBe(threadId);
	...
	it("revokes draft image blob URLs when clearing a matching project draft thread by id", () => {
		...
		try {
			store.setProjectDraftThreadId(projectRef, draftId, { threadId });
			store.addImage(draftId, makeImage({ id: "img-project-clear-by-id", previewUrl: "blob:clear-by-id" }));
	...
	it("clears orphaned composer drafts when remapping a project to a new draft thread", () => {
		...
		expect(useComposerDraftStore.getState().getDraftThreadByProjectRef(projectRef)?.threadId).toBe(otherThreadId);
		expect(useComposerDraftStore.getState().getDraftThread(draftId)).toBeNull();
		expect(draftByKey(draftId)).toBeUndefined();
	});
	...
	it("keeps composer drafts when the thread is still mapped by another project", () => {
		...
		expect(useComposerDraftStore.getState().getDraftThreadByProjectRef(projectRef)).toBeNull();
		expect(useComposerDraftStore.getState().getDraftThreadByProjectRef(otherProjectRef)?.threadId).toBe(threadId);
	...
	it("marks promoted drafts from an iterable of server thread ids", () => {
		...
		expect(draftByKey(draftId)?.prompt).toBe("promote me");
		expect(useComposerDraftStore.getState().getDraftThreadByProjectRef(otherProjectRef)?.threadId).toBe(
			otherThreadId,
		);
...
describe("composerDraftStore modelSelection", () => {
	...
	it("keeps explicit off/default codex overrides on the selection", () => {
		...
		store.setProviderModelOptions(threadRef, "codex", toSelections({ reasoningEffort: "high", fastMode: false }));
	...
	it("updates only the draft when sticky persistence is omitted", () => {
		...
		store.setStickyModelSelection(modelSelection("claudeAgent", "claude-opus-4-6", { effort: "max" }));
		store.setModelSelection(threadRef, modelSelection("claudeAgent", "claude-opus-4-6", { effort: "max" }));
	...
	it("does not clear other provider options when setting options for a single provider", () => {
		...
		expect(draft?.modelSelectionByProvider.claudeAgent?.options).toEqual(
			createModelSelection("claudeAgent", "claude-opus-4-6", toSelections({ effort: "max" })).options,
		);
	});
